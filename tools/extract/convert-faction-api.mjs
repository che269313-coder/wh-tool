...
if (!slug || !faction) throw new Error("用法：node tools/extract/convert-faction-api.mjs --slug <slug> --name <简体队伍名>");
...
fs.writeFileSync(sourceNotePath, [
  ...
  `- 简体文件：${faction}-网站原始数据-简体.json（由 tools/extract/ConvertTo-SimplifiedChinese.ps1 生成）`,
  `- 结构化文件：${faction}-结构化数据卡.json（由 tools/extract/convert-faction-api.mjs 生成）`,
