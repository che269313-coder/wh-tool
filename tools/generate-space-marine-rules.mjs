...
const phaseFor = (text) => {
  ...
  if (/射击阶段|远程攻击|射击攻击|射击武器|进行射击|射击目标|目标射击|射击步兵/.test(compact)) return "ranged";
  return undefined;
};
...
function effectDescriptors(text) {
  ...
  const add = (effect) => result.push(phaseFor(text) ? { ...effect, phase: phaseFor(text) } : effect);
  ...
  if (/重投近战攻击命中结果中的1[^。]*重投特殊保护投掷结果中的1/.test(compact)) {
    return { effects: [
      { type: "hit-reroll", mode: "ones", phase: "melee", selection: { controlId: "mode", equals: "hit-ones", fallback: "none" } },
      { type: "save-reroll", mode: "ones", phase: "melee", selection: { controlId: "mode", equals: "save-ones", fallback: "none" } },
    ], leader: false };
  }
  const mortalFnp = text.match(/遭受致命伤害攻击\s*时拥有【不\s*知\s*疼\s*痛\s*([3456])\s*\+】/);
  const psychicFnp = compact.match(/(?:对抗|遭到)[^。]*灵能[^。]*不(?:知|觉)疼痛([3456])\+/);
  if (psychicFnp) {
    const threshold = Number(psychicFnp[1]);
    add({ type: "fnp", threshold, selection: { controlId: "incomingPsychic", equals: true } });
    if (/致命伤害/.test(compact)) add({ type: "fnp-mortal", threshold, activation: "passive" });
  } else if (mortalFnp) {
  ...
  if (/爆弹步枪\s*A\s*[+＋]\s*2/.test(text)) {
    const generalIndex = result.findIndex((effect) => effect.type === "attack-modifier" && effect.value === 2);
    if (generalIndex >= 0) result.splice(generalIndex, 1);
    add({ type: "weapon-attack-modifier", weaponName: "爆弹步枪", value: 2 });
  }
  if (/近战武器\s*S\s*[+＋]\s*1/.test(text)) add({ type: "weapon-strength-modifier", value: 1 });
  if (/近战攻击[^。\n]*冲锋[^。\n]*S\s*和\s*D\s*都\s*[+＋]\s*1/.test(text)) {
    const selection = { controlId: "charged", equals: true };
    add({ type: "weapon-strength-modifier", value: 1, selection });
    add({ type: "damage-modifier", value: 1, selection });
  }
  ...
  if (invulnerable) add({ type: "invulnerable-save", value: Number(invulnerable[1]), ...(/一次性|可以|获得|本阶段|持续/.test(text) ? { operation: "override" } : {}) });
  if (/攻城指挥官|攻击巨兽[，,、 ]*载具[，,、 ]*工事单位时[\s\S]*S[，, ]*AP[，, ]*D[都均]?增强\s*2/.test(text)) {
    const selection = { controlId: "targetMonsterVehicle", equals: true };
    add({ type: "weapon-strength-modifier", value: 2, selection });
    add({ type: "weapon-ap-modifier", value: 2, selection });
    add({ type: "damage-modifier", value: 2, selection });
  }
...
function toRule(card, text, index) {
  ...
  const unsupportedExternalTarget = [
    "铸造之主", "机神祝福", "铸造之父", "多重光谱阵列", "雷霆轰炸", "冰雹轰炸", "长者智慧",
  ...
  const targetKeywordControls = [
    ...(declaredEffects.some((effect) => effect.requiresTargetMonsterVehicle || effect.selection?.controlId === "targetMonsterVehicle")
    ...
    ...(declaredEffects.some((effect) => effect.condition === "targetBelowHalf")
      ...
      : []),
    ...(declaredEffects.some((effect) => effect.selection?.controlId === "charged")
      ? [{ id: "charged", type: "checkbox", label: "本单位本回合进行过冲锋移动" }]
      : []),
    ...(declaredEffects.some((effect) => effect.selection?.controlId === "incomingPsychic")
      ? [{ id: "incomingPsychic", semanticType: "incoming-psychic", type: "checkbox", label: "来袭攻击具有【灵能】关键词" }]
      : []),
    ...(declaredEffects.some((effect) => effect.selection?.controlId === "mode")
      ? [{ id: "mode", type: "select", label: "本阶段选择", options: [["none", "不启用"], ["hit-ones", "重投近战命中结果中的 1"], ["save-ones", "重投特殊保护结果中的 1"]] }]
      : []),
  ];
  ...
  return {
    ...
    ...(declaredEffects.length ? { ...((passiveInvulnerableSave || passiveFeelNoPain || passiveDamageHalving) ? {} : { controls: targetKeywordControls.length ? [...targetKeywordControls, ...(leader ? controls(true).slice(1) : [])] : [...controls(leader), ...strengthConditionControls] }), effects: declaredEffects } : {}),
...
const oathIdentity = {
  ...
  legacyIds: ["space-marines-oath-of-moment"],
};
const factionRules = [{
  id: oathIdentity.id,
  legacyIds: oathIdentity.legacyIds,
  identity: oathIdentity,
  name: "破敌重誓",
  text: oathText,
  status: "计算支持（命中重投默认生效，造伤加成由通用控件处理）",
  controls: [{ id: "woundBonus", type: "checkbox", label: "本次对破敌重誓目标造伤 +1" }],
  effects: [
    { type: "hit-reroll", mode: "failed", activation: "passive" },
    { type: "wound-modifier", value: 1, selection: { controlId: "woundBonus", equals: true } },
  ],
  source: { file: "分遣队规则-可检索.md", url: oathIdentity.sourceUrl },
}];
