...
// ── 断言锚点 ────────────────────────────────────────────────────────────────
// 中文显示名会随 PDF 卡面名裁决更新（欧克兽人改名事件见 repair-logs/），
// 断言必须锚定稳定身份：规则 id 或 source.englishName。
// 中文显示名只能作为断言的"输出"（校验裁决结果），不得作为查找的"输入"。
const ruleById = (rulesGlobal, ruleId) =>
  Object.values(rulesGlobal.unitRules || {}).flat().find((rule) => rule.id === ruleId)
  || (rulesGlobal.factionRules || []).find((rule) => rule.id === ruleId);
const unitEntryByRuleId = (rulesGlobal, ruleId) => {
  const entry = Object.entries(rulesGlobal.unitRules || {}).find(([, rules]) => rules.some((rule) => rule.id === ruleId));
  assert(entry, `断言锚点丢失：找不到规则 ${ruleId}（显示名可能已被裁决更新，请核对稳定 ID）`);
  return { unitName: entry?.[0], rules: entry?.[1] || [] };
};
const unitEntryByEnglishName = (rulesGlobal, englishName) => {
  const entry = Object.entries(rulesGlobal.unitRules || {}).find(([, rules]) => rules.some((rule) => rule.source?.englishName === englishName));
  assert(entry, `断言锚点丢失：找不到 englishName 为 ${englishName} 的单位技能`);
  return { unitName: entry?.[0], rules: entry?.[1] || [] };
};
const unitRuleById = (entry, ruleId) => entry.rules.find((rule) => rule.id === ruleId);
const unitRuleByEnglishName = (entry, englishName) => entry.rules.find((rule) => rule.source?.englishName === englishName);
...
assert(ruleById(context.WarhammerCustodesRules, "adeptus-custodes.aleya.tenacious-spirit"), "坚毅灵魂必须使用官方英文 Tenacious Spirit ID");
...
const beastbossEntry = unitEntryByEnglishName(context.WarhammerOrksRules, "Beastboss");
const waagh = context.WarhammerRuleResolver.resolveFaction("欧克兽人", { "orks.army.waaagh.enabled": true }, { phase: "melee", unitName: beastbossEntry.unitName });
...
const beastboss = unitRuleByEnglishName(beastbossEntry, "Beastboss");
const beastbossResolved = context.WarhammerRuleResolver.resolveUnit("欧克兽人", beastbossEntry.unitName, {}, { phase: "melee", isJoined: true });
assert(beastboss?.effects?.some((effect) => effect.type === "hit-modifier" && effect.value === 1 && effect.requiresJoined), "欧克兽人兽霸头目（Beastboss）必须声明领导单位时近战命中 +1");
assert(beastbossResolved.attack.hitModifier === 1, "欧克兽人兽霸头目领导单位时近战命中 +1 必须进入计算");
const ferociousRage = unitRuleByEnglishName(beastbossEntry, "Ferocious Rage");
const ferociousResolved = context.WarhammerRuleResolver.resolveUnit("欧克兽人", beastbossEntry.unitName, { [`${ferociousRage?.id}.charged`]: true }, { phase: "melee" });
...
const bigMekEntry = unitEntryByEnglishName(context.WarhammerOrksRules, "More Dakka");
const moreDakka = unitRuleByEnglishName(bigMekEntry, "More Dakka");
const moreDakkaResolved = context.WarhammerRuleResolver.resolveUnit("欧克兽人", bigMekEntry.unitName, { [`${moreDakka?.id}.enabled`]: true }, { phase: "ranged", isJoined: true });
...
const tankbustasEntry = unitEntryByEnglishName(context.WarhammerOrksRules, "Tank Hunters");
const orksTankHunters = unitRuleByEnglishName(tankbustasEntry, "Tank Hunters");
const tankResolved = context.WarhammerRuleResolver.resolveUnit("欧克兽人", tankbustasEntry.unitName, { [`${orksTankHunters?.id}.targetMonsterVehicle`]: true }, { phase: "ranged" });
...
assert(ruleById(context.WarhammerSpaceMarineRules, "space-marines.captain.rites-of-battle"), "星际战士连长战斗之仪必须使用官方 Rites of Battle ID");
assert(ruleById(context.WarhammerSpaceMarineRules, "space-marines.captain-with-jump-pack.rites-of-battle")?.name === "战斗之仪", "跳跃背包连长技能顺序不得把战斗之仪错配为 Angel's Wrath");
assert(ruleById(context.WarhammerSpaceMarineRules, "space-marines.captain-with-jump-pack.angels-wrath")?.name === "天使之怒", "跳跃背包连长天使之怒必须使用官方 Angel's Wrath ID");
assert(ruleById(context.WarhammerSpaceMarineRules, "space-marines.drop-pod.drop-pod-assault")?.name === "空降突击", "空降仓技能必须在官网能力数量不一致时仍映射 Drop Pod Assault");
assert(ruleById(context.WarhammerSpaceMarineRules, "space-marines.hammerfall-bunker.fortification")?.name === "防御工事", "落锤堡能力必须按语义覆盖映射，不能仅按数组顺序配对");
assert(ruleById(context.WarhammerDeathGuardRules, "death-guard.typhus.destroyer-hive"), "泰弗斯毁灭虫群必须使用官方 Destroyer Hive ID");
...
const blightlordEntry = unitEntryByRuleId(deathGuardRules, "death-guard.blightlord-terminators.blistering-fusillade");
const blightlordVolley = unitRuleById(blightlordEntry, "death-guard.blightlord-terminators.blistering-fusillade");
...
const blightHaulerEntry = unitEntryByRuleId(deathGuardRules, "death-guard.myphitic-blight-haulers.tank-hunters");
const tankHunters = unitRuleById(blightHaulerEntry, "death-guard.myphitic-blight-haulers.tank-hunters");
...
const contagionCasterEntry = unitEntryByRuleId(deathGuardRules, "death-guard.malignant-plaguecaster.gift-of-contagion");
const contagionGift = unitRuleById(contagionCasterEntry, "death-guard.malignant-plaguecaster.gift-of-contagion");
...
const defilerDamaged = unitRuleById(defilerEntry, "death-guard.defiler.seriously-damaged");
...
const mortarionCore = ruleById(deathGuardRules, "death-guard.mortarion.core-abilities");
...
if (contagionGift) {
  const contagionEnabled = resolve("死亡守卫", contagionCasterEntry.unitName, {
...
const genericCaptainEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.captain.rites-of-battle");
const genericCaptainRules = genericCaptainEntry.rules;
assert(genericCaptainRules.some((rule) => rule.id === "space-marines.captain.rites-of-battle"), "通用人物数据卡必须显示战斗之仪");
const peakMoment = unitRuleById(genericCaptainEntry, "space-marines.captain.finest-hour");
...
const guardOnObjective = resolve("帝皇禁军", custodianGuardName, { "custodes-guard-stance.onObjective": true }, { phase: "melee" });
assert(guardOnObjective.attack.woundReroll === "failed", "禁军盾卫在己方目标点必须重投失败造伤");

const trajannEntry = unitEntryByRuleId(context.WarhammerCustodesRules, "adeptus-custodes.trajann-valoris.moment-shackle");
const trajannAxe = resolve("帝皇禁军", trajannEntry.unitName, { "custodes-trajan-time-lock.mode": "axe" }, { phase: "melee" });
...
const trajannInvulnerable = resolve("帝皇禁军", trajannEntry.unitName, { "custodes-trajan-time-lock.mode": "invulnerable" }, { phase: "melee" });
...
const trajannInvulnerableRanged = resolve("帝皇禁军", trajannEntry.unitName, { "custodes-trajan-time-lock.mode": "invulnerable" }, { phase: "ranged" });
assert(trajannInvulnerableRanged.defend.invulnerableSave === 0, "时间枷锁的 2+ 特殊保护只能在近战阶段生效");

const custodianVolley = unitRuleById(custodianGuardEntry, "adeptus-custodes.custodian-guard.sentinel-storm");
const custodianVolleyMelee = resolve("帝皇禁军", custodianGuardName, { [`${custodianVolley?.id}.enabled`]: true }, { phase: "melee" });
const custodianVolleyRanged = resolve("帝皇禁军", custodianGuardName, { [`${custodianVolley?.id}.enabled`]: true }, { phase: "ranged" });
...
const shieldCaptainEntry = unitEntryByRuleId(context.WarhammerCustodesRules, "adeptus-custodes.shield-captain.master-of-the-stances");
const shieldCaptainMartialMaster = unitRuleById(shieldCaptainEntry, "adeptus-custodes.shield-captain.master-of-the-stances");
const standaloneSecondKatah = resolve("帝皇禁军", shieldCaptainEntry.unitName, { [`${shieldCaptainMartialMaster?.id}.secondKatah`]: "sustained" }, { phase: "melee", isJoined: false });
...
const aleyaEntry = unitEntryByRuleId(context.WarhammerCustodesRules, "adeptus-custodes.aleya.tenacious-spirit");
const aleyaForcedLeader = resolve("帝皇禁军", aleyaEntry.unitName, { "custodes-aleya-soul.forceLeader": true }, { phase: "melee", isJoined: false });
...
const allarusTerminators = resolve("帝皇禁军", allarusEntry.unitName, {}, { phase: "melee" });
const vigilators = resolve("帝皇禁军", vigilatorsEntry.unitName, {}, { phase: "melee" });
...
const agathonEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.adrax-agatone.heavy-blow");
const agathonHeavyBlow = unitRuleById(agathonEntry, "space-marines.adrax-agatone.heavy-blow");
const agathonMelee = resolve("星际战士", agathonEntry.unitName, { [`${agathonHeavyBlow?.id}.enabled`]: true, [`${agathonHeavyBlow?.id}.forceLeader`]: true }, { phase: "melee", isJoined: false });
const agathonRanged = resolve("星际战士", agathonEntry.unitName, { [`${agathonHeavyBlow?.id}.enabled`]: true, [`${agathonHeavyBlow?.id}.forceLeader`]: true }, { phase: "ranged", isJoined: false });
...
const outriderEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.outrider-squad.thunderous-impact");
const outriderImpact = unitRuleById(outriderEntry, "space-marines.outrider-squad.thunderous-impact");
const outriderImpactSelections = { [`${outriderImpact?.id}.enabled`]: true, [`${outriderImpact?.id}.charged`]: true };
const outriderImpactMelee = resolve("星际战士", outriderEntry.unitName, outriderImpactSelections, { phase: "melee" });
const outriderImpactRanged = resolve("星际战士", outriderEntry.unitName, outriderImpactSelections, { phase: "ranged" });
...
const thorEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.tor-garadon.siege-commander");
const thor = resolve("星际战士", thorEntry.unitName, {}, { phase: "melee" });
assert(thorRule && !thorRule.controls?.length && thor.defend.invulnerableSave === 4, "托尔连长的4+特殊保护必须无控件且自动进入防御结算");
for (const hoodId of ["space-marines.librarian.psychic-hood", "space-marines.librarian-in-phobos-armour.psychic-hood", "space-marines.librarian-in-terminator-armour.psychic-hood"]) {
  const hoodEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, hoodId);
  const hood = unitRuleById(hoodEntry, hoodId);
  const normal = resolve("星际战士", hoodEntry.unitName, { [`${hood?.id}.forceLeader`]: true }, { phase: "ranged", isJoined: false });
  const psychic = resolve("星际战士", hoodEntry.unitName, { [`${hood?.id}.forceLeader`]: true, [`${hood?.id}.incomingPsychic`]: true }, { phase: "ranged", isJoined: false });
  assert(normal.defend.feelNoPain === 0 && psychic.defend.feelNoPain === 4, `${hoodEntry.unitName}灵能头冠只能在来袭攻击具有灵能关键词时提供不知疼痛4+`);
}
const tiguriusEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.chief-librarian-tigurius.hood-of-hellfire");
const hellfireHood = unitRuleById(tiguriusEntry, "space-marines.chief-librarian-tigurius.hood-of-hellfire");
const hellfireNormal = resolve("星际战士", tiguriusEntry.unitName, { [`${hellfireHood?.id}.forceLeader`]: true }, { phase: "ranged", isJoined: false });
const hellfirePsychic = resolve("星际战士", tiguriusEntry.unitName, { [`${hellfireHood?.id}.forceLeader`]: true, [`${hellfireHood?.id}.incomingPsychic`]: true }, { phase: "ranged", isJoined: false });
...
const lysanderEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.darnath-lysander.paragon-of-fortitude");
const lysanderSaveRule = unitRuleById(lysanderEntry, "space-marines.darnath-lysander.invulnerable-save");
const lysanderImperviousRule = unitRuleById(lysanderEntry, "space-marines.darnath-lysander.indomitable");
const lysanderBase = resolve("星际战士", lysanderEntry.unitName, {}, { phase: "melee" });
const lysanderImpervious = resolve("星际战士", lysanderEntry.unitName, { [`${lysanderImperviousRule?.id}.enabled`]: true }, { phase: "melee" });
...
const siegeCommander = unitRuleById(thorEntry, "space-marines.tor-garadon.siege-commander");
const thorAgainstInfantry = resolve("星际战士", thorEntry.unitName, {}, { phase: "melee" });
const thorAgainstVehicle = resolve("星际战士", thorEntry.unitName, { [`${siegeCommander?.id}.targetMonsterVehicle`]: true }, { phase: "melee" });
...
const standardBearerEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.ancient-in-terminator-armour.keep-the-banner-high");
const standardBearerRule = unitRuleById(standardBearerEntry, "space-marines.ancient-in-terminator-armour.keep-the-banner-high");
const standardBearerSelections = { [`${standardBearerRule?.id}.enabled`]: true, [`${standardBearerRule?.id}.forceLeader`]: true };
const standardBearerNormal = standardBearerRule && resolve(spaceMarineData.faction, standardBearerEntry.unitName, standardBearerSelections, { phase: "melee", isJoined: true, underStartingStrength: false, belowHalfStrength: false });
const standardBearerUnderStarting = standardBearerRule && resolve(spaceMarineData.faction, standardBearerEntry.unitName, standardBearerSelections, { phase: "melee", isJoined: true, underStartingStrength: true, belowHalfStrength: false });
const standardBearerBelowHalf = standardBearerRule && resolve(spaceMarineData.faction, standardBearerEntry.unitName, standardBearerSelections, { phase: "melee", isJoined: true, underStartingStrength: true, belowHalfStrength: true });
...
const ironFatherEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.iron-father-feirros.master-of-the-forge");
const ironFatherForgeMaster = unitRuleById(ironFatherEntry, "space-marines.iron-father-feirros.master-of-the-forge");
const techmarineEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.techmarine.blessing-of-the-omnissiah");
const techmarineBlessing = unitRuleById(techmarineEntry, "space-marines.techmarine.blessing-of-the-omnissiah");
...
for (const [fnpEntry, threshold] of [
  [unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.captain-titus.relentless-assault"), 5],
  [unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.vulkan-he-stan.father-of-the-forge"), 6],
  [unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.pedro-kantor.oath-of-the-lion"), 6],
]) {
  const core = fnpEntry.rules.find((rule) => rule.effects?.some((effect) => effect.type === "fnp" && effect.threshold === threshold));
  assert(core && !core.controls?.length, `${fnpEntry.unitName}核心不知疼痛必须默认生效`);
}
const lysanderFortitude = unitRuleById(lysanderEntry, "space-marines.darnath-lysander.paragon-of-fortitude");
const lysanderEnabled = resolve("星际战士", lysanderEntry.unitName, { [`${lysanderFortitude?.id}.enabled`]: true, [`${lysanderFortitude?.id}.forceLeader`]: true }, { phase: "melee", isJoined: false });
assert(lysanderFortitude?.effects?.some((effect) => effect.type === "incoming-wound-when-strength-gte") && lysanderEnabled.defend.incomingWoundWhenStrengthGreaterOrEqual === -1, "坚毅典范必须把 S≥T 的造伤 -1 传入防守方效果");
...
const lionOathEnabled = resolve("星际战士", kantorEntry.unitName, { [`${lionOath?.id}.enabled`]: true, [`${lionOath?.id}.forceLeader`]: true }, { phase: "melee", isJoined: false });
...
const jumpCaptainEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.captain-with-jump-pack.angels-wrath");
...
const gravisCaptainEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.captain-in-gravis-armour.refuse-to-yield");
const neverYieldRule = unitRuleById(gravisCaptainEntry, "space-marines.captain-in-gravis-armour.refuse-to-yield");
const neverYield = resolve("星际战士", gravisCaptainEntry.unitName, {}, { phase: "melee", isJoined: false });
...
const varEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.caanok-var.cold-calculation");
const kanoCore = varEntry.rules.find((rule) => rule.effects?.some((effect) => effect.type === "fnp" && effect.threshold === 5));
assert(kanoCore && !kanoCore.controls?.length, "卡诺克.瓦核心不知疼痛5+必须默认生效（不觉疼痛）");
const coldCalculus = unitRuleById(varEntry, "space-marines.caanok-var.cold-calculation");
...
for (const litanyId of ["space-marines.chaplain.litany-of-hate", "space-marines.chaplain-in-terminator-armour.litany-of-hate", "space-marines.chaplain-on-bike.litany-of-hate", "space-marines.chaplain-with-jump-pack.litany-of-hate"]) {
  const priestEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, litanyId);
  const hatred = unitRuleById(priestEntry, litanyId);
  assert(hatred?.effects?.some((effect) => effect.type === "wound-modifier" && effect.value === 1), `${priestEntry.unitName}憎恨祷言必须提供近战造伤+1`);
}
const terminatorPriestEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.chaplain-in-terminator-armour.recitation-of-faith");
const faith = unitRuleById(terminatorPriestEntry, "space-marines.chaplain-in-terminator-armour.recitation-of-faith");
...
const biologisEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.apothecary-biologis.surgical-precision");
assert(biologisEntry.rules.some((rule) => rule.effects?.some((effect) => effect.type === "lethal-hits")), "手术式精准必须提供致命一击");
const ancientEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.ancient.unbreakable-duty");
const undying = unitRuleById(ancientEntry, "space-marines.ancient.unbreakable-duty");
assert(undying?.effects?.some((effect) => effect.type === "fnp" && effect.threshold === 4), "旗手不休职责必须提供条件性不知疼痛4+");
const honourGuardEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.victrix-honour-guard.ultramarines-honour-guard");
const honorGuard = unitRuleById(honourGuardEntry, "space-marines.victrix-honour-guard.ultramarines-honour-guard");
assert(honorGuard?.effects?.some((effect) => effect.type === "incoming-wound-minus"), "极限战士荣誉卫队必须提供被领导时的造伤-1");
const macragge = unitRuleById(honourGuardEntry, "space-marines.victrix-honour-guard.banner-of-macragge");
...
const heavyIntercessorEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.heavy-intercessor-squad.unyielding-in-the-face-of-the-foe");
const stubborn = unitRuleById(heavyIntercessorEntry, "space-marines.heavy-intercessor-squad.unyielding-in-the-face-of-the-foe");
assert(stubborn?.effects?.some((effect) => effect.type === "save-bonus-vs-d1"), "拒不屈服必须提供对抗 D1 攻击的护甲+1");
const aggressorEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.aggressor-squad.close-quarters-firepower");
const closeRange = unitRuleById(aggressorEntry, "space-marines.aggressor-squad.close-quarters-firepower");
assert(closeRange?.effects?.some((effect) => effect.type === "weapon-ap-modifier" && effect.value === 1), "近距离火力必须提供 AP+1");
const eradicatorEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.eradicator-squad.total-obliteration");
const totalAnnihilation = unitRuleById(eradicatorEntry, "space-marines.eradicator-squad.total-obliteration");
...
const terminatorSquadEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.terminator-squad.fury-of-the-first");
const chainRage = unitRuleById(terminatorSquadEntry, "space-marines.terminator-squad.fury-of-the-first");
assert(chainRage?.effects?.some((effect) => effect.type === "hit-modifier" && effect.value === 1), "一连之怒必须通过通用命中修正提供对破敌重誓目标的命中+1");
const suppressorEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.suppressor-squad.suppression-fire");
const suppressor = unitRuleById(suppressorEntry, "space-marines.suppressor-squad.suppression-fire");
assert(suppressor?.effects?.some((effect) => effect.type === "incoming-hit-minus" && !effect.phase), "压制火力不能被限定为仅远程");
const redemptorEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.redemptor-dreadnought.duty-eternal");
const eternalDuty = unitRuleById(redemptorEntry, "space-marines.redemptor-dreadnought.duty-eternal");
const stormravenEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.stormraven-gunship.armoured-resilience");
const toughArmor = unitRuleById(stormravenEntry, "space-marines.stormraven-gunship.armoured-resilience");
...
const predatorDestructorEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.predator-destructor.destructor");
const saboteur = unitRuleById(predatorDestructorEntry, "space-marines.predator-destructor.destructor");
assert(saboteur?.effects?.some((effect) => effect.type === "ap-vs-infantry" && effect.requiresTargetInfantry), "破坏者必须提供对步兵的 AP+1");
const predatorAnnihilatorEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.predator-annihilator.annihilator");
const annihilator = unitRuleById(predatorAnnihilatorEntry, "space-marines.predator-annihilator.annihilator");
assert(annihilator?.effects?.some((effect) => effect.type === "damage-reroll" && effect.requiresTargetMonsterVehicle), "歼灭者必须提供对巨兽/载具的破坏力重投");
const executionerEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.repulsor-executioner.executioner");
const executioner = unitRuleById(executionerEntry, "space-marines.repulsor-executioner.executioner");
assert(executioner?.effects?.some((effect) => effect.type === "hit-modifier" && effect.condition === "targetBelowHalf"), "处决者必须提供对低于半数目标的命中+1");
const razorbackEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.razorback.fire-support");
const porcupine = unitRuleById(razorbackEntry, "space-marines.razorback.fire-support");
...
const bladeguardAncientEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.bladeguard-ancient.deeds-of-heroism");
const heroics = unitRuleById(bladeguardAncientEntry, "space-marines.bladeguard-ancient.deeds-of-heroism");
...
const bladeguardVeteranEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.bladeguard-veteran-squad.bladeguard");
const bladeguard = unitRuleById(bladeguardVeteranEntry, "space-marines.bladeguard-veteran-squad.bladeguard");
const bladeguardHit = resolve("星际战士", bladeguardVeteranEntry.unitName, { [`${bladeguard?.id}.mode`]: "hit-ones" }, { phase: "melee" });
const bladeguardSave = resolve("星际战士", bladeguardVeteranEntry.unitName, { [`${bladeguard?.id}.mode`]: "save-ones" }, { phase: "melee" });
...
const intercessorEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.intercessor-squad.hail-of-bolts");
const objectiveAnnihilation = unitRuleById(intercessorEntry, "space-marines.intercessor-squad.hail-of-bolts");
...
const objectiveAnnihilationResolved = resolve("星际战士", intercessorEntry.unitName, { [`${objectiveAnnihilation?.id}.enabled`]: true }, { phase: "ranged" });
...
for (const rangedOnlyId of ["space-marines.aggressor-squad.close-quarters-firepower", "space-marines.eradicator-squad.total-obliteration", "space-marines.ballistus-dreadnought.ballistus-strike", "space-marines.predator-destructor.destructor"]) {
  const rangedOnlyEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, rangedOnlyId);
  const rule = unitRuleById(rangedOnlyEntry, rangedOnlyId);
  assert(rule?.effects?.every((effect) => effect.phase === "ranged"), `${rangedOnlyEntry.unitName}·${rule?.name} 的攻击效果必须限定为远程阶段`);
}
const dreadnoughtEntry = unitEntryByRuleId(context.WarhammerSpaceMarineRules, "space-marines.dreadnought.wisdom-of-the-ancients");
const wisdom = unitRuleById(dreadnoughtEntry, "space-marines.dreadnought.wisdom-of-the-ancients");
...
const typhusEntry = unitEntryByRuleId(context.WarhammerDeathGuardRules, "death-guard.typhus.destroyer-hive");
const typhusDefend = resolve("死亡守卫", typhusEntry.unitName, { "death-guard-p25-1.enabled": true, "death-guard-p25-1.forceLeader": true }, { phase: "melee", isJoined: false });
assert(typhusDefend.defend.incomingHitModifier === -1, "泰弗斯毁灭虫群必须提供近战命中-1");
const chaosRhinoEntry = unitEntryByRuleId(context.WarhammerDeathGuardRules, "death-guard.chaos-rhino.fire-support");
const chaosRhinoFireSupport = unitRuleById(chaosRhinoEntry, "death-guard.chaos-rhino.fire-support");
assert(chaosRhinoFireSupport?.effects?.some((effect) => effect.type === "wound-reroll")
  && !chaosRhinoFireSupport?.effects?.some((effect) => effect.type === "hit-reroll"), "犀牛火力支援只能提供造伤重投");
const tallymanEntry = unitEntryByRuleId(context.WarhammerDeathGuardRules, "death-guard.tallyman.malicious-calculations");
const scribe = unitRuleById(tallymanEntry, "death-guard.tallyman.malicious-calculations");
assert(scribe?.effects?.some((effect) => effect.type === "ignore-hit-modifiers"), "书记官恶意计算必须提供无视命中修正");
const deathshroudEntry = unitEntryByRuleId(context.WarhammerDeathGuardRules, "death-guard.deathshroud-terminators.silent-bodyguard");
const silent = unitRuleById(deathshroudEntry, "death-guard.deathshroud-terminators.silent-bodyguard");
...
assert(ruleById(context.WarhammerDeathGuardRules, "death-guard.deathshroud-terminators.death-approaches")
  && silent?.id === "death-guard.deathshroud-terminators.silent-bodyguard", "死亡寿衣终结者已存在的官网英文技能必须优先生成 official 稳定 ID");
assert(ruleById(context.WarhammerDeathGuardRules, "death-guard.plague-marines.infused-with-the-blessings-of-nurgle"), "瘟疫战士官网技能必须优先于本地暂译名匹配");
const helbruteEntry = unitEntryByRuleId(context.WarhammerDeathGuardRules, "death-guard.helbrute.diseased-malice");
const malice = unitRuleById(helbruteEntry, "death-guard.helbrute.diseased-malice");
assert(malice?.effects?.some((effect) => effect.type === "wound-modifier" && effect.requiresTargetInfected), "疫病恶意必须要求目标已感染");
const greatUncleanEntry = unitEntryByRuleId(context.WarhammerDeathGuardRules, "death-guard.great-unclean-one.nurgles-rot");
const virulent = unitRuleById(greatUncleanEntry, "death-guard.great-unclean-one.nurgles-rot");
assert(virulent?.effects?.some((effect) => effect.type === "target-toughness-modifier" && effect.value === -1), "纳垢之腐必须提供目标T-1");
const rotigusEntry = unitEntryByRuleId(context.WarhammerDeathGuardRules, "death-guard.rotigus.toxic-blessing");
const poison = unitRuleById(rotigusEntry, "death-guard.rotigus.toxic-blessing");
assert(poison?.effects?.some((effect) => effect.type === "damage-modifier" && effect.value === 1), "剧毒赐福必须提供伤害D+1");
const nurglingsEntry = unitEntryByRuleId(context.WarhammerDeathGuardRules, "death-guard.nurglings.mischief-makers");
const trickster = unitRuleById(nurglingsEntry, "death-guard.nurglings.mischief-makers");
...
const lordOfVirulenceEntry = unitEntryByRuleId(context.WarhammerDeathGuardRules, "death-guard.lord-of-virulence.virulent-aura");
const virulentAura = unitRuleById(lordOfVirulenceEntry, "death-guard.lord-of-virulence.virulent-aura");
const virulentSelections = { [`${virulentAura?.id}.enabled`]: true, [`${virulentAura?.id}.forceLeader`]: true };
assert(resolve("死亡守卫", lordOfVirulenceEntry.unitName, virulentSelections, { phase: "ranged", isJoined: false }).attack.woundReroll === "failed"
  && resolve("死亡守卫", lordOfVirulenceEntry.unitName, virulentSelections, { phase: "melee", isJoined: false }).attack.woundReroll === null, "病毒光环的造伤重投只能影响远程攻击");
const blightlordVolleySelections = { [`${blightlordVolley?.id}.enabled`]: true, [`${blightlordVolley?.id}.targetInfected`]: true };
assert(resolve("死亡守卫", blightlordEntry.unitName, blightlordVolleySelections, { phase: "ranged", initialModelCount: 5 }).attack.strengthModifier === 1
  && resolve("死亡守卫", blightlordEntry.unitName, blightlordVolleySelections, { phase: "melee", initialModelCount: 5 }).attack.strengthModifier === 0, "炽烈连射的武器加成只能影响远程攻击");
const tankHunterSelections = { [`${tankHunters?.id}.enabled`]: true, [`${tankHunters?.id}.targetMonsterVehicle`]: true };
assert(resolve("死亡守卫", blightHaulerEntry.unitName, tankHunterSelections, { phase: "ranged" }).attack.hitModifier === 1
  && resolve("死亡守卫", blightHaulerEntry.unitName, tankHunterSelections, { phase: "melee" }).attack.hitModifier === 0, "坦克猎手的命中与造伤加成只能影响远程攻击");
const plagueSurgeonEntry = unitEntryByRuleId(context.WarhammerDeathGuardRules, "death-guard.plague-surgeon.inflamed-infections");
const inflamedInfections = unitRuleById(plagueSurgeonEntry, "death-guard.plague-surgeon.inflamed-infections");
const inflamedSelections = { [`${inflamedInfections?.id}.enabled`]: true, [`${inflamedInfections?.id}.targetBelowHalf`]: true };
const inflamedMelee = resolve("死亡守卫", plagueSurgeonEntry.unitName, inflamedSelections, { phase: "melee" });
const inflamedRanged = resolve("死亡守卫", plagueSurgeonEntry.unitName, inflamedSelections, { phase: "ranged" });
...
const prosecutorPurity = unitRuleById(prosecutorEntry, "adeptus-custodes.prosecutors.purity-of-execution");
const puritySelections = { [`${prosecutorPurity?.id}.enabled`]: true, [`${prosecutorPurity?.id}.targetPsychic`]: true };
assert(resolve("帝皇禁军", prosecutorEntry.unitName, puritySelections, { phase: "ranged" }).attack.devastating
  && !resolve("帝皇禁军", prosecutorEntry.unitName, puritySelections, { phase: "melee" }).attack.devastating, "纯净处决的毁灭伤害只能影响远程攻击");
