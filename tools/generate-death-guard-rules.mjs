...
function phaseFor(text) {
  ...
  if (/射击阶段|远程攻击|射击攻击|射击武器/.test(compact)) return "ranged";
  return undefined;
}
...
function effectDescriptors(text) {
  ...
  if (hitRerollText.test(text)) add({ type: "hit-reroll", mode: /(?:重掷|重投)\s*命中(?:结?\s*果)?\s*[1一]|命中(?:结?\s*果)?(?:中)?的?\s*[1一]/.test(text) ? "ones" : "failed" });
  if (woundRerollText.test(text)) add({ type: "wound-reroll", mode: /(?:重掷|重投)\s*造伤(?:结?\s*果)?\s*[1一]/.test(text) ? "ones" : "failed" });
  ...
  if (infectedWoundBonus) {
    add({ type: "wound-modifier", value: 1, requiresTargetInfected: true });
    customControls.push(...targetControls);
  }
  if (/命\s*中\s*与\s*造伤骰\s*结果\s*各\s*[+＋]\s*1/.test(text)) {
    add({ type: "hit-modifier", value: 1, requiresTargetMonsterVehicle: true });
    add({ type: "wound-modifier", value: 1, requiresTargetMonsterVehicle: true });
    customControls.push(...monsterVehicleControls);
  }
  if (/力\s*量\s*S\s*与\s*穿甲值\s*AP\s*各\s*[+＋]\s*1/.test(text)) {
    add({ type: "weapon-strength-modifier", value: 1, condition: "large-or-led", requiresTargetInfected: true });
    add({ type: "weapon-ap-modifier", value: 1, condition: "large-or-led", requiresTargetInfected: true });
    customControls.push(...targetControls);
  }
  if (/传染馈赠/.test(text) && /连\s*击\s*1/.test(text)) {
    add({ type: "sustained-hits", value: 1, requiresTargetInfected: true });
    customControls.push(...targetControls);
  ...
  else if (meleeHitMinus) add({ type: "incoming-hit-minus", value: 1, phase: "melee" });
  ...
  if (/命中5\+触发暴击.{0,100}低于其一半的初始数量.{0,30}4\+/.test(text)) {
    add({ type: "hit-critical-threshold", value: 5, belowHalfValue: 4, condition: "targetBelowHalf" });
...
const factionRules = [{
  ...
  controls: [
    ...
    { id: "plague", type: "select", label: "额外瘟疫（三选一）", options: [["none", "不选择"], ["skullsquirm", "扭颅疫病：近战攻击命中 -1"], ["rattlejoint", "颤骨瘟疫：保护值 SV -1"], ["scabrous", "烂魂伤风：移动/领导/OC -1"]] },
  ],
  effects: [
