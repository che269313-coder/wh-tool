...
2. 单位名称不确定时先调用 find_units；需要单位属性、武器或技能时调用 get_unit_profile。工具找不到数据时如实追问，不得编造。
...
6. 工具返回后优先基于返回内容作答；不可因重复调用相同工具而把内部调用上限暴露给用户。若仍无法回答，要用自然语言说明还缺什么。
7. 回答先给结论，再简要说明采用的单位、攻击方式、明确条件、资料或工具依据，以及仍缺少的关键条件。
    ...
    toolDefinitions: [
      {
        ...
        function: {
          name: "find_units",
          description: "在当前军表和项目内置数据卡中查找单位；名称不确定时先调用它。",
          parameters: {
            type: "object",
      ...
      },
      {
        ...
        function: {
          ...
          parameters: {
            type: "object",
            properties: { name: { type: "string" }, side: { type: "string", enum: ["attacker", "defender"] } },
            required: ["name"],
          },
        },
      },
