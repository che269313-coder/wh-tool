...
(function (root) {
  const targets = Object.freeze({
    ...
    "灵能者": "psyker", "灵能": "psyker", psyker: "psyker", "人物": "character", character: "character",
  ...
  function numericEffects(text, pattern, type, fallback = 0) {
    return [...text.matchAll(pattern)].map((match) => {
      ...
      return { type, value: raw === "d3" ? raw : Number(raw || fallback) };
    });
  }
  ...
  function parseSegment(value) {
    ...
    effects.push(...numericEffects(text, /(?:速射|rapid\s*fire)\s*(d3|\d+)/gi, "rapid-fire"));
    effects.push(...numericEffects(text, /(?:热熔|melta)\s*(d3|\d+)/gi, "melta"));
    ...
    for (const match of text.matchAll(/(?:反|针对)[- ]?(步兵|载具|巨兽|凶兽|工事|泰坦级|飞行|灵能者|灵能|人物)\s*([2-6])\+|anti[- ]?(infantry|vehicle|monster|fortification|titanic|flying|fly|psyker|character)\s*([2-6])\+/gi)) {
      const label = String(match[1] || match[3]).toLowerCase();
      effects.push({ type: "anti-keyword", target: targets[label], threshold: Number(match[2] || match[4]) });
    }
    const suffix = text.match(/[：:]\s*([^：:]+)$/);
    if (!suffix) return effects;
    const targetKeywords = suffix[1]
      .split(/[\/、]/)
      .map((label) => String(label).trim().toLowerCase())
      .map((label) => targets[label] || label)
      .filter(Boolean);
    return targetKeywords.length ? effects.map((effect) => ({ ...effect, targetKeywords })) : effects;
  }
  ...
  const targetAliases = Object.freeze({
    ...
    psyker: ["灵能者", "灵能", "psyker"], character: ["人物", "character"],
