/* Generated source-text rule package for astra-militarum. */
(function (root) {
  root["WarhammerWebsiteRules_astra_militarum"] = {
  "factionRules": [
    {
      "id": "astra-militarum.army-rule",
      "name": "号令之声",
      "text": "你军队中具有此能力的军官可下达命令。军官的资料卡会载明每回合可下达几道命令、哪些单位有资格接收。每次下达时，从下方挑一道命令，并指定该军官 6\" 内一个有资格的友军单位接收。\n\n军官可在你的指挥阶段、或在他们下载具／部署的阶段结束时下达命令。\n\n被指定的单位在下个指挥阶段前受该命令影响。除非另有说明，一个单位同时只能受一道命令影响（后下达者取代前者）。战斗震慑单位无法接收命令；受命令影响的单位若进入战斗震慑，该命令立即失效。\n\n只有帝国卫队模型能享有命令效益。\n\n前进！前进！前进！\n\n移动 +3\"。\n\n装上刺刀！\n\n近战武器武技 +1。\n\n瞄准！\n\n远程武器准确度 +1。\n\n前排，开火！后排，开火！\n\n快速射击武器攻击次数 +1。\n\n寻求掩护！\n\n豁免 +1（不可优于 3+）。\n\n责任与荣誉！\n\n领导力与目标控制各 +1。",
      "status": "仅供查阅",
      "effects": []
    }
  ],
  "unitRules": {
    "神盾防线墙": [
      {
        "id": "unique-emplacement-platform",
        "name": "驻紮平台",
        "text": "友军 **星界军(Astra Militarum)步兵(Infantry)** 模型可以在此 **防御工事(Fortification)** 的平台部分上设置或结束任何类型的移动。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-reinforced-cover",
        "name": "加固掩护",
        "text": "每当一次远程攻击被分配给一个模型时，若该模型因此 **防御工事(Fortification)** 对发动攻击的单位内的每个模型都不完全可见，则该模型对该攻击获得掩护优势。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-defence-line",
        "name": "防线",
        "text": "当 **星界军(Astra Militarum)步兵(Infantry)** 模型因此地形特征而获得掩护庇护时，该模型具有 4+ 无敌豁免掷骰。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-fortification",
        "name": "防御工事",
        "text": "当敌方单位仅在你的军队中一个或多个 **防御工事(Fortification)** 的交战范围内时：\n\n■ 该单位仍可被选为远程攻击的目标，但每次进行此类攻击时，除非该攻击是用手枪进行，否则从命中掷骰减 1。\n■ 该单位中的模型在战斗震撼时后撤时无需进行绝望逃脱测试，除非它们在此过程中会越过敌方模型。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-deployment",
        "name": "部署",
        "text": "此模型部署时,由 1 个平台段、至多 5 个护盾段、至多 2 个破损护盾段与至多 2 个末端段构成。所有段落必须彼此相连以形成连续的防线;2 个破损护盾段可置于防线末端,或置于防线中段使两者彼此相距 ?\" 内(此时此 2 段视为彼此相连)。所有已部署的段落就所有规则目的而言视为单一模型。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "装甲哨兵": [
      {
        "id": "core-deadly-demise-1",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-mobile-hunter-killers",
        "name": "移动猎杀者",
        "text": "此单位中的模型每次对 **凶兽(Monster)** 或 **载具(Vehicle)** 单位发动攻击时，你可重掷致伤掷骰。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 任意数量的模型可各自用下列之一替换其多管雷射炮：\n? 1 自动炮\n? 1 重型火焰喷射器\n? 1 雷射加农炮\n? 1 飞弹发射器\n? 1 电浆炮",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 任意数量的模型各自可装备 1 把哨兵链锯。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 任意数量的模型可各自配备 1 枚猎杀飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "炮兵小组": [
      {
        "id": "unique-remorseless-barrage",
        "name": "无情弹幕",
        "text": "在你的射击阶段，此模型射击后，若其用曲射武器进行的一次或更多攻击对敌方单位造成命中，该单位必须进行战斗震撼检测（若 **步兵(Infantry)** 单位被多管火箭发射器进行的一次或更多攻击命中，进行检测时战斗震撼检测减 1）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战术装备能力",
        "text": "■ 此模型的重迫炮可替换为下列之一：\n? 1 门攻城炮\n? 1 门重型四联发射器\n? 1 门多管火箭发射器",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "复仇者攻击机": [
      {
        "id": "core-deadly-demise-d6",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-fiery-vengeance",
        "name": "炽烈复仇",
        "text": "每个回合一次，在你对手的射击阶段中，当距离此模型 6\" 内的另一个友方 **帝国卫队** 单位因能 **飞行(Fly)** 的单位进行的攻击而被摧毁时，你军队中有此能力的一个模型可以使用它。若其如此做，在进攻单位完成其攻击后，该模型可以射击，就如同处于你的射击阶段一般，但在解决这些攻击时只能以该敌方单位为目标（且只有当它是合法目标时才能选择）。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "阿提拉蛮骑兵": [
      {
        "id": "unique-horsemasters",
        "name": "马术大师",
        "text": "此单位在进行后撤的回合中可以进行射击并宣告冲锋。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装配备能力",
        "text": "■ 此单位每 5 个模型中，1 个模型的狩猎矛可替换为 1 支催逐矛。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装装备能力",
        "text": "■ 蛮骑中士可配备 1 动力军刀。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "毒刃": [
      {
        "id": "core-deadly-demise-d6-2",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-rolling-fortress",
        "name": "滚动堡垒",
        "text": "每当一次远程攻击分配给你军队中的一个 **星界军(Astra Militarum)** 模型，如果该模型因此 **毁灭之刃超重坦克** 模型而不被攻击单位中的每个模型完全看到，该模型对该攻击具有掩体优势。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型的 2 具双联重型火焰喷射器可替换为 2 具双联重型爆弹枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 此模型可配备以下之一：\n? 2门雷射加农炮和2门双联重爆弹枪\n? 2门雷射加农炮和2门双联重型火焰喷射器",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "毁灭之剑": [
      {
        "id": "core-deadly-demise-d6-2",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-armour-obliteration",
        "name": "装甲粉碎",
        "text": "每当以此模型的震地炮发动的攻击摧毁具有致命破灭的敌方模型时，该模型的致命破灭在 D6 掷骰为 3+ 时而非 6 时造成致命伤。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武器配备能力",
        "text": "■ 此模型的2门双联重型火焰枪可替换为2门双联重型火枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型可配备以下其中之一：\n? 2个雷射加农炮和2个双联重爆弹枪\n? 2个雷射加农炮和2个双联重型火焰喷射器",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "毁灭之锤": [
      {
        "id": "core-deadly-demise-d6-2",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-firing-deck-6",
        "name": "射击甲板6",
        "text": "某些 **运输工具(Transport)** 模型在其能力中列有「射击甲板 x」。每次在射击阶段选择此类模型射击时，你可选择最多「x」个搭乘其中且其单位本阶段尚未射击的搭乘模型。然后，对于每个选中的搭乘模型，你可选择该搭乘模型配备的一个远程武器（不包括具有 **[ONE SHOT]** 能力的武器）。在该 **运输工具(Transport)** 模型解决其所有攻击前，它在计算时被视为额外配备你选择的所有武器。至本阶段结束，这些选中模型的单位没有资格射击。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-tremor-quake",
        "name": "地震颤动",
        "text": "在你的射击阶段，刚选择此模型震荡炮的目标后，该目标单位和所有在该单位3\"内的其他敌方 **步兵(Infantry)** 单位必须进行战斗震撼测试。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-transport",
        "name": "运载",
        "text": "该模型能够搭载 26 个星界军步兵模型。每个 OGRYN 模型占用 3 个模型的空间。该模型不能运输 ARTILLERY 模型。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备装备能力",
        "text": "■ 此模型的 2 门双联重型火焰喷射器可替换为 2 门双联重爆弹枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 此模型可装备以下其中之一：\n? 2 重型雷射炮和 2 双联重型机枪\n? 2 重型雷射炮和 2 双联重型火焰枪",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "石化蜥蜴自行火炮": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-earthshaker-rounds",
        "name": "地震者炮弹",
        "text": "在你的射击阶段，此模型射击后，如果其地震炮发动的一次或多次攻击对敌方步兵单位命中，直到你的下一个射击阶段开始，该单位被震撼。单位被震撼时，从其移动特性中扣除 2\"，且其冲锋掷骰扣除 2。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 此模型的重爆弹枪可替换为 1 个重型火焰喷射器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型可配备 1 个猎杀飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "牛格林小队": [
      {
        "id": "core-feel-no-pain-6",
        "name": "不觉疼痛6+",
        "text": "此能力一律以「不觉疼痛 X+」的形式呈现。每当具有此能力的模型将要失去一个伤值时，掷一次 D6：掷出 X+ 时，该伤值不会失去。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-wall-of-muscle",
        "name": "肌肉之墙",
        "text": "此单位每次被分配伤害时，将该攻击的伤害特性减少1。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武器配备能力",
        "text": "■ 任意数量的模型各可将其掷弹兵手甲替换为 1 条 Bullgryn 巨锤。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 任意数量模型可各自将其厚重盾牌替换为 1 野蛮盾牌。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-brute-shield",
        "name": "蛮力盾",
        "text": "持有者拥有 4+ 无敌豁免(InSv)。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-slabshield",
        "name": "巨板盾",
        "text": "持有者的伤口(W)特性为 4。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "卡迪亚堡主": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为**星界军(Astra Militarum)**，具有此能力的**军官(Officer)**可下达命令。每份**军官(Officer)**资料卡会指定其在一个战斗回合中可下达多少命令，以及哪些单位可接收该命令。每当一个**军官(Officer)**模型下达命令时，选择下方的命令之一，然后选择该**军官(Officer)**模型6寸内的一个合格友军单位来接收命令。\n\n**军官(Officer)**模型可在你的指挥阶段或其从**运输工具(Transport)**工具下车或在战场上布置的阶段结束时下达命令。\n\n直至你下一个指挥阶段开始为止，你选择的单位受到该命令影响。除非另有说明，一个单位一次只能受到一个命令影响（随后下达给该单位的任何命令取代现有命令）。命令不能下达给陷入战斗震撼的单位，若受命令影响的单位陷入战斗震撼，该命令对该单位的影响停止。\n\nOnly **星界军(Astra Militarum)** 模型得益于下达给其单位的命令。\n\n前进！前进！前进！\n\n向此单位中的模型的移动特性增加3寸。\n\n刺刀上膛！\n\n提升此单位中由模型配备的近战武器的武器技巧特性1点。\n\n瞄准！\n\n提升此单位中由模型配备的远程武器的巴利斯蒂克技巧特性1点。\n\n第一排，开火！第二排，开火！\n\n提升此单位中由模型配备的速射武器的攻击次数特性1点。\n\n寻求掩护！\n\n提升此单位中模型的豁免特性1点（此无法将模型的豁免提升至优于3+）。\n\n职责与荣誉！\n\n提升此单位中模型的领导力和目标控制特性各1点。\n\n经过训练以灌输绝对服从，帝国警卫的军官拥有司令权和尊重，以及能够穿过战争喧嚣发出的威严号令。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-senior-officer",
        "name": "资深军官",
        "text": "当此模型率领一个单位时，该单位中模型装备的远程武器具有 **[SUSTAINED HITS 1]** 能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-get-back-in-the-fight",
        "name": "重返战斗",
        "text": "当此模型领导一个单位时，该单位在于该回合后撤时仍可射击。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此军官每回合最多可下达 2 道命令给 REGIMENT 单位。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型的链锯剑可替换为以下之一：\n? 1 把栓动枪和 1 件近战武器\n? 1 把力量拳\n? 1 把力量武器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型的las手枪可替换为以下其中之一：\n? 1 把弹药手枪\n? 1 把血浆手枪",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "卡迪亚指挥组": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为 **星界军(Astra Militarum)，军官(Officer)** 可以下令。每份 **军官(Officer)** 数据表会说明它在一个战斗回合中能下多少令，以及哪些单位可以接收该令。每次 **军官(Officer)** 模型下令时，选择下列其中一个令，然后选择该 **军官(Officer)** 模型 6\" 内符合资格的一个友方单位接收该令。\n\n**军官(Officer)** 模型可在你的指挥阶段和它们从 **运输工具(Transport)** 中下车或被部署在战场上的阶段结束时下令。\n\n直到你的下一个指挥阶段开始为止，你选择的单位会受到该令的影响。除非另有说明，一个单位同时只能受一个令的影响（随后下达给该单位的任何令会替代现有的令）。惊吓战斗状态的单位不能接收令，如果受令单位陷入惊吓状态，该令对该单位的影响会停止。\n\n只有 **星界军(Astra Militarum)** 模型能获得下达给其单位的令的效益。\n\n冲啊！冲啊！冲啊！\n\n此单位模型的移动特征加上 3\"。\n\n刺刀冲锋！\n\n此单位模型装备的近战武器的武器技巧特征提升 1。\n\n瞄准射击！\n\n此单位模型装备的远程武器的弹道技巧特征提升 1。\n\n前列射击！后列射击！\n\n此单位模型装备的快射武器的攻击次数特征提升 1。\n\n掩蔽！\n\n此单位模型的豁免(Sv)特性提升 1（此不能将模型的豁免提升至优于 3+）。\n\n职责与荣誉！\n\n此单位模型的领导力和目标控制特征提升 1。\n\n帝国卫队军官经历培训，旨在灌输不动摇的服从。他们掌握指挥权力与尊重，以傲慢的咆哮下达令命，其声音刺穿战争的喧嚣。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-cadia-stands",
        "name": "卡迪亚屹立！",
        "text": "当此单位包含一个**军官(Officer)**模型，且此单位在一个**目标标记**的范围内时，此单位可重掷**战斗震慑掷骰(battle-shock rolls)**。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此单位的军官每回合可下达 1 道命令给一个 REGIMENT 单位。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战甲能力",
        "text": "■ 1 名卡迪亚退伍军人卫兵的雷射枪和团体标准可被替换为下列其中之一：\n? 1 把火焰枪\n? 1 把榴弹发射器\n? 1 把熔火枪\n? 1 把电浆枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装装备能力",
        "text": "■ 1 名卡迪亚老兵枪兵的拉枪可替换为下列其一：\n? 1 把栓动手枪\n? 1 把电浆手枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战斗装备能力",
        "text": "■ 卡迪亚指挥官的火焰手枪可被替换为以下其中之一：\n? 1 把鱼飣栓动手枪\n? 1 把电浆手枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战斗装备能力",
        "text": "■ 1 个卡迪亚老兵卫兵的电锯剑可替换为以下之一：\n? 1 具火焰枪与 1 件近战武器\n? 1 具手雷发射器与 1 件近战武器\n? 1 具熔火枪与 1 件近战武器\n? 1 具电浆枪与 1 件近战武器\n? 1 把力量拳\n? 1 件力量武器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 凯迪亚指挥官的锯齿剑可替换为以下之一：\n? 1 把力场拳套\n? 1 把力场武器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-master-vox",
        "name": "主通讯机",
        "text": "持有者所在的单位中的 **军官(Officer)** 每次下达命令时，可以对至多 24\" 外的合格单位下达命令。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-medi-pack",
        "name": "医疗包",
        "text": "在你的指挥阶段开始时，若持有者所在的单位低于其起始兵力，你可以将至多 D3 个被消灭的 **PLATOON** 模型（不含 **角色(Character)**）放回此单位。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-regimental-standard",
        "name": "军团军旗",
        "text": "持有者所在的单位中的模型其目标控制(OC)特性 +1。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "卡迪亚重型武器小队": [
      {
        "id": "unique-covering-fire",
        "name": "掩护射击",
        "text": "每次使用火力监视策略对此单位进行目标标定时，解决该策略期间，命中在未修正命中掷骰 5+ 时成功，或在此单位位于一个或多个友军 **排** 单位 6\" 范围内时，改为未修正命中掷骰 4+ 时成功。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-embarking",
        "name": "搭乘",
        "text": "搭乘于运输工具(Transport)内时,每个模型占用 2 个模型的空间;就射击甲板(Firing Deck)能力而言,这些模型所配备的每件武器均视为 2 个模型的武器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战甲能力",
        "text": "■ 任意数量的模型可各自将其重型闇骑枪替换为以下其中一项：\n? 1 支自动炮\n? 1 支雷射枪\n? 1 个飞弹发射器\n? 1 门迫击炮",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "卡迪亚侦查小队": [
      {
        "id": "core-infiltrators",
        "name": "渗透",
        "text": "在部署期间，若一个单位中的每个模型都具有此能力，则可将其设置在战场上任何距离你对手的部署区与所有敌方单位水平超过 8\" 的位置。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-stealth",
        "name": "潜行",
        "text": "若一个单位中的每个模型都具有此能力，则每当有远程攻击以该单位为目标时，该单位在该攻击中具有**掩体增益**（13.08）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-independent-operatives",
        "name": "独立行动",
        "text": "每次对此单位下达命令（Order）时，该命令会持续影响此单位，直到对此单位下达新命令、或此单位陷入战斗震撼、或战斗结束为止（以先发生者为准）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "装备能力",
        "text": "■ 一名侦查兵可将其雷射步枪替换为 1 把自动连射枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "装备能力",
        "text": "■ 一名侦查兵可将其雷射步枪替换为以下其一：\n? 1 把电浆枪\n? 1 把热熔枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "装备能力",
        "text": "■ 至多一名侦查兵可将其雷射步枪替换为 1 把远距雷射枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "装备能力",
        "text": "■ 一名配备 1 把雷射步枪的侦查兵可额外配备 1 具通讯兵（vox-caster）与 1 具无线电中继信标（该模型的雷射步枪不可被替换）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "装备能力",
        "text": "■ 你可选择以下其一：\n? 至多一名侦查兵可将其雷射步枪替换为 1 把远距雷射枪。\n? 至多两名配备雷射步枪的侦查兵可组成武器小组；若如此，一名模型的雷射步枪替换为 1 把雷射手枪，另一名模型的雷射步枪替换为 1 具飞弹发射器与 1 把雷射手枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-vox-caster",
        "name": "通讯机",
        "text": "你每次以计谋指定持有者所在的单位为目标时，掷一颗 D6，若 6\" 内有一个或多个友军 **OFFICER** 模型则结果 +1：若为 5+，你获得 1CP。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-vox-relay-beacon",
        "name": "通讯中继信标",
        "text": "在战斗开始时，你可以为此单位在战场上任何不属于对手部署区的位置设置一个 Vox-relay Beacon 标记。当友军 **ASTRA MILITARUM REGIMENT** 单位在该标记 6\" 内时，你军队中的 **OFFICER** 单位可对该 **REGIMENT** 单位发布命令，如同该 **OFFICER** 单位在其 6\" 内一般。在每个阶段结束时，若有敌方单位在你设置的 Vox-relay Beacon 标记 1\" 内，则该标记从战场上移除。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "卡迪亚突击军": [
      {
        "id": "unique-shock-troops",
        "name": "突击部队",
        "text": "在你的指挥阶段结束时，若此单位在你控制的一个目标标记的范围内，该目标标记在你对手在一个阶段结束时对该目标标记的控制等级大于你的控制等级之前，保持在你的控制下。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 此单位每 10 个模型，最多 2 个冲击士兵可各自将其雷射枪替换为下列其一：*\n? 1 把火焰枪\n? 1 把榴弹发射器\n? 1 把熔火枪\n? 1 把电浆枪\n\n* 除非单位包含 20 个模型（此时不得选择超过 2 次同一武器），否则每个单位不得选择超过 1 次同一武器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 此单位每10个模型，1个配备雷射枪的冲击兵可配备1部通讯器（该模型的雷射枪无法被替换）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 任意数量的冲击部队下士可各自将其拉枪手枪替换为 1 把栓动手枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 任意数量的冲击部队士官可各自将其雷射手枪和链刃替换为 1 士官自动枪和 1 近战武器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-vox-caster",
        "name": "通讯机",
        "text": "你每次以计谋指定持有者所在的单位为目标时，掷一颗 D6，若 6\" 内有一个或多个友军 **OFFICER** 模型则结果 +1：若为 5+，你获得 1CP。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "卡塔昌指挥组": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-scouts-6",
        "name": "侦察6\"",
        "text": "此能力一律以「斥候 X\"」的形式呈现。在结算战前能力步骤中，若一个单位中的每个模型都具有此能力，你可以执行下列其中**一**项：\n? 若该单位位于**战略预备队(strategic reserves)**中，你可以将该单位设置于完全位于你部署区内的任意位置。\n? 若该单位完全位于你的部署区内，它可以进行一次**斥候移动(scout move)**（24.32）。\n? 若该单位搭载于一个完全位于你部署区内的**专用运输工具(Dedicated Transport)**内，且搭载于该专用运输工具内的每个模型都具有斥候能力，则该专用运输工具可以进行一次斥候移动。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为 **星界军(Astra Militarum)，军官(Officer)** 可以下令。每份 **军官(Officer)** 数据表会说明它在一个战斗回合中能下多少令，以及哪些单位可以接收该令。每次 **军官(Officer)** 模型下令时，选择下列其中一个令，然后选择该 **军官(Officer)** 模型 6\" 内符合资格的一个友方单位接收该令。\n\n**军官(Officer)** 模型可在你的指挥阶段和它们从 **运输工具(Transport)** 中下车或被部署在战场上的阶段结束时下令。\n\n直到你的下一个指挥阶段开始为止，你选择的单位会受到该令的影响。除非另有说明，一个单位同时只能受一个令的影响（随后下达给该单位的任何令会替代现有的令）。惊吓战斗状态的单位不能接收令，如果受令单位陷入惊吓状态，该令对该单位的影响会停止。\n\n只有 **星界军(Astra Militarum)** 模型能获得下达给其单位的令的效益。\n\n冲啊！冲啊！冲啊！\n\n此单位模型的移动特征加上 3\"。\n\n刺刀冲锋！\n\n此单位模型装备的近战武器的武器技巧特征提升 1。\n\n瞄准射击！\n\n此单位模型装备的远程武器的弹道技巧特征提升 1。\n\n前列射击！后列射击！\n\n此单位模型装备的快射武器的攻击次数特征提升 1。\n\n掩蔽！\n\n此单位模型的豁免(Sv)特性提升 1（此不能将模型的豁免提升至优于 3+）。\n\n职责与荣誉！\n\n此单位模型的领导力和目标控制特征提升 1。\n\n帝国卫队军官经历培训，旨在灌输不动摇的服从。他们掌握指挥权力与尊重，以傲慢的咆哮下达令命，其声音刺穿战争的喧嚣。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-gung-ho-command",
        "name": "勇气指令",
        "text": "当此单位含有 **军官(Officer)** 时，此单位中模型装备的远程武器具有 **[突击]** 能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此单位的军官每回合可下达 1 道命令给一个 REGIMENT 单位。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 任意数量的资深卫兵可各自将其半自动雷射枪替换为以下之一*：\n? 1 火焰枪\n? 1 榴弹发射器\n? 1 重型火焰枪\n? 1 熔火枪\n? 1 电浆枪\n? 1 狙击步枪\n\n* *你每个单位最多只能从此列表中选择一种武器。*",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 1 名配备雷射枪的资深卫兵可配备 1 台主通话机。**",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "军装能力",
        "text": "■ 1 个配备有雷射枪的退伍老兵卫队可配备 1 个医疗包。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 1名配备雷射枪的资深后卫可配备1面团队军旗。**\n\n** *一个模型只能选择其中一项。*",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ Catachan 指挥官的las手枪可被替换为下列其一：\n? 1 把博尔特手枪\n? 1 把电浆手枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 卡塔开指挥官可配备下列其中之一：\n? 1 把栓动步枪\n? 1 把链锯剑\n? 1 个力量拳\n? 1 件力量武器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-master-vox",
        "name": "主通讯机",
        "text": "持有者所在的单位中的 **军官(Officer)** 每次下达命令时，可以对至多 24\" 外的合格单位下达命令。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-medi-pack",
        "name": "医疗包",
        "text": "在你的指挥阶段开始时，若持有者所在的单位低于其起始兵力，你可以将至多 D3 个被消灭的 **PLATOON** 模型（不含 **角色(Character)**）放回此单位。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-regimental-standard",
        "name": "军团军旗",
        "text": "持有者所在的单位中的模型其目标控制(OC)特性 +1。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "卡塔昌重型武器小队": [
      {
        "id": "core-scouts-6",
        "name": "侦察6\"",
        "text": "此能力一律以「斥候 X\"」的形式呈现。在结算战前能力步骤中，若一个单位中的每个模型都具有此能力，你可以执行下列其中**一**项：\n? 若该单位位于**战略预备队(strategic reserves)**中，你可以将该单位设置于完全位于你部署区内的任意位置。\n? 若该单位完全位于你的部署区内，它可以进行一次**斥候移动(scout move)**（24.32）。\n? 若该单位搭载于一个完全位于你部署区内的**专用运输工具(Dedicated Transport)**内，且搭载于该专用运输工具内的每个模型都具有斥候能力，则该专用运输工具可以进行一次斥候移动。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-bring-it-down",
        "name": "击倒它！",
        "text": "此单位中的模型每次以远程攻击对 **凶兽(Monster)** 或 **载具(Vehicle)** 单位发动攻击时，重掷命中掷骰为 1，并重掷致伤掷骰为 1。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-embarking",
        "name": "搭乘",
        "text": "搭乘于运输工具(Transport)内时,每个模型占用 2 个模型的空间;就射击甲板(Firing Deck)能力而言,这些模型所配备的每件武器均视为 2 个模型的武器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 任意数量的模型可各自将其重型博尔特枪替换为下列其一：\n? 1 挺自动炮\n? 1 挺las炮\n? 1 具飞弹发射器\n? 1 门迫击炮",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "卡塔昌丛林斗士": [
      {
        "id": "core-scouts-6",
        "name": "侦察6\"",
        "text": "此能力一律以「斥候 X\"」的形式呈现。在结算战前能力步骤中，若一个单位中的每个模型都具有此能力，你可以执行下列其中**一**项：\n? 若该单位位于**战略预备队(strategic reserves)**中，你可以将该单位设置于完全位于你部署区内的任意位置。\n? 若该单位完全位于你的部署区内，它可以进行一次**斥候移动(scout move)**（24.32）。\n? 若该单位搭载于一个完全位于你部署区内的**专用运输工具(Dedicated Transport)**内，且搭载于该专用运输工具内的每个模型都具有斥候能力，则该专用运输工具可以进行一次斥候移动。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-jungle-fighters",
        "name": "丛林战士",
        "text": "此单位中的模型每次发动近战攻击时，如果此单位在此回合进行了冲锋移动或被冲锋，致伤掷骰 +1。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 每 5 名此单位的丛林战士的拉枪，可替换 1 支为火焰枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "装备能力",
        "text": "■ 此单位每 10 个模型，就有 1 个配备雷射枪的丛林战斗士可配备 1 部通话器（该模型的雷射枪不可被替换）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-vox-caster",
        "name": "通讯机",
        "text": "你每次以计谋指定持有者所在的单位为目标时，掷一颗 D6，若 6\" 内有一个或多个友军 **OFFICER** 模型则结果 +1：若为 5+，你获得 1CP。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "半人马快速突击战车": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-firing-deck-12",
        "name": "射击甲板12",
        "text": "某些**运输工具(Transport)**模型在其能力中列有「射击甲板 x」。每次该模型在射击阶段被选中进行射击时，你可以选择最多 x 个其单位本阶段尚未射击的已登载模型。然后，对每个已选中的登载模型，你可以选择该登载模型配备的一件远程武器（不包括具有**[单发]**能力的武器）。在该**运输工具(Transport)**模型解决所有攻击前，除其他武器外，它还计为配备所有你选中的武器。直到本阶段结束，该些已选中模型的单位不符合射击资格。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-rapid-strike-vehicle",
        "name": "快速打击载具",
        "text": "当有一个或多个单位搭载于此模型内时，除非此模型处于战斗震撼状态，否则此模型的「目标控制」特征值每每搭载其中的 3 个模型（向下取整）就加 1。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-transport",
        "name": "运载",
        "text": "该模型能够搭载 12 个星界军步兵模型。每个 OGRYN 模型占用 3 个模型的空间。该模型不能运输 ARTILLERY 模型。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "奇美拉装甲车": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-firing-deck-2",
        "name": "射击甲板2",
        "text": "某些 **运输工具(Transport)** 模型的能力中列有「射击甲板 x」。每当这类模型在射击阶段被选中射击时，你可以选择最多 x 个搭乘其内的模型，其所属的单位本阶段尚未射击。然后，对于这些搭乘模型中的每一个，你可以选择该搭乘模型配备的一件远程武器（不包括具有 **[单发]** 能力的武器）。在那个 **运输工具(Transport)** 模型完成其所有攻击之前，它视为配备有所有你以此方式选择的武器，除其他武器外。直到阶段末，那些被选中的模型所属的单位不适格射击。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-mobile-command-vehicle",
        "name": "机动指挥车",
        "text": "在你的指挥阶段，搭乘于此 **运输工具(Transport)** 内的一个 **军官(Officer)** 模型可以下达命令，尽管它不在战场上。进行此操作时，测量距离到此 **运输工具(Transport)** 以及从此 **运输工具(Transport)** 的距离。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-transport",
        "name": "运载",
        "text": "该模型能够搭载 12 个星界军步兵模型。每个 OGRYN 模型占用 3 个模型的空间。该模型不能运输 ARTILLERY 模型。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型的重爆弹枪可替换为 1 重型火焰喷射器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装装备能力",
        "text": "■ 此模型的多功能雷射枪可替换为下列其一：\n? 1 挺重型栓动枪*\n? 1 挺重型火焰枪*\n\n* *此武器无法被替换。*",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 此模型可配备下列其中之一：\n? 1个重型机枪\n? 1个风暴爆弹枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 此模型可配备 1 猎杀者飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "政委": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为**星界军(Astra Militarum)**，具有此能力的**军官(Officer)**可下达命令。每份**军官(Officer)**资料卡会指定其在一个战斗回合中可下达多少命令，以及哪些单位可接收该命令。每当一个**军官(Officer)**模型下达命令时，选择下方的命令之一，然后选择该**军官(Officer)**模型6寸内的一个合格友军单位来接收命令。\n\n**军官(Officer)**模型可在你的指挥阶段或其从**运输工具(Transport)**工具下车或在战场上布置的阶段结束时下达命令。\n\n直至你下一个指挥阶段开始为止，你选择的单位受到该命令影响。除非另有说明，一个单位一次只能受到一个命令影响（随后下达给该单位的任何命令取代现有命令）。命令不能下达给陷入战斗震撼的单位，若受命令影响的单位陷入战斗震撼，该命令对该单位的影响停止。\n\nOnly **星界军(Astra Militarum)** 模型得益于下达给其单位的命令。\n\n前进！前进！前进！\n\n向此单位中的模型的移动特性增加3寸。\n\n刺刀上膛！\n\n提升此单位中由模型配备的近战武器的武器技巧特性1点。\n\n瞄准！\n\n提升此单位中由模型配备的远程武器的巴利斯蒂克技巧特性1点。\n\n第一排，开火！第二排，开火！\n\n提升此单位中由模型配备的速射武器的攻击次数特性1点。\n\n寻求掩护！\n\n提升此单位中模型的豁免特性1点（此无法将模型的豁免提升至优于3+）。\n\n职责与荣誉！\n\n提升此单位中模型的领导力和目标控制特性各1点。\n\n经过训练以灌输绝对服从，帝国警卫的军官拥有司令权和尊重，以及能够穿过战争喧嚣发出的威严号令。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-summary-execution",
        "name": "即决处刑",
        "text": "每战斗回合一次，在任何阶段开始时，你可选择一个友方 **星界军(Astra Militarum)步兵(Infantry)** 单位，其已遭战斗震撼且在此模型 12\" 内。若你如此做，该单位中的一个模型被摧毁，该单位之后不再遭战斗震撼。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-political-overwatch",
        "name": "政治监察",
        "text": "另一个 **军官(Officer)** 模型与此模型在同一单位中时，可重掷该单位的「战斗震撼」测试。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此军官每回合可下达 1 道命令给一个 REGIMENT 单位。此军官只能下达「责任与荣誉！」和「装上刺刀！」命令。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 此模型的爆弹手枪可替换为 1 个电浆手枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 此模型的锯齿剑可替换为 1 把力场武器。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "独眼巨人爆破车": [
      {
        "id": "core-deadly-demise-1",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-demolition-charges",
        "name": "爆破炸药",
        "text": "每当此模型或一个敌方单位完成移动时，若此模型在一个或多个敌方单位的 3\" 范围内，选择其中一个敌方单位。此模型被摧毁，但不为其致命破灭能力掷骰，改为掷一次 D6：掷出 2-5 时，该敌方单位承受 D3 点致命伤；掷出 6 时，该敌方单位承受 D6 点致命伤。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-unstable-payload",
        "name": "不稳定弹药",
        "text": "为此模型的致命破灭能力掷骰时，6\" 内的单位在 4+ 时受到致命伤，而非 6。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-compact",
        "name": "小巧",
        "text": "此模型可如同步兵(Infantry)模型一般搭乘于 ASTRA MILITARUM TRANSPORT 模型内。若如此,它占用 7 个模型的空间。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "格雷夫斯政委": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为 **星界军(Astra Militarum)，军官(Officer)** 可以下令。每份 **军官(Officer)** 数据表会说明它在一个战斗回合中能下多少令，以及哪些单位可以接收该令。每次 **军官(Officer)** 模型下令时，选择下列其中一个令，然后选择该 **军官(Officer)** 模型 6\" 内符合资格的一个友方单位接收该令。\n\n**军官(Officer)** 模型可在你的指挥阶段和它们从 **运输工具(Transport)** 中下车或被部署在战场上的阶段结束时下令。\n\n直到你的下一个指挥阶段开始为止，你选择的单位会受到该令的影响。除非另有说明，一个单位同时只能受一个令的影响（随后下达给该单位的任何令会替代现有的令）。惊吓战斗状态的单位不能接收令，如果受令单位陷入惊吓状态，该令对该单位的影响会停止。\n\n只有 **星界军(Astra Militarum)** 模型能获得下达给其单位的令的效益。\n\n冲啊！冲啊！冲啊！\n\n此单位模型的移动特征加上 3\"。\n\n刺刀冲锋！\n\n此单位模型装备的近战武器的武器技巧特征提升 1。\n\n瞄准射击！\n\n此单位模型装备的远程武器的弹道技巧特征提升 1。\n\n前列射击！后列射击！\n\n此单位模型装备的快射武器的攻击次数特征提升 1。\n\n掩蔽！\n\n此单位模型的豁免(Sv)特性提升 1（此不能将模型的豁免提升至优于 3+）。\n\n职责与荣誉！\n\n此单位模型的领导力和目标控制特征提升 1。\n\n帝国卫队军官经历培训，旨在灌输不动摇的服从。他们掌握指挥权力与尊重，以傲慢的咆哮下达令命，其声音刺穿战争的喧嚣。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-mechanised-spearhead",
        "name": "机械化先锋",
        "text": "在你的移动阶段，每次友方 **帝国国防军 阵营** 单位从距离此模型6\"内的 **运输工具(Transport)** 上下船时，在该单位部署后，此模型可向该 **阵营** 单位下达1道命令，不受此模型本轮已下达多少命令的限制。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-brutal-disciplinarian",
        "name": "残酷纠察官",
        "text": "每回合一次，在任何阶段开始时，你可以选择一个友军 **星界军(Astra Militarum)步兵(Infantry)** 单位（仅包含一个模型的单位除外），该单位被战斗震撼且在此模型 24\" 内且可见。若你这样做，该单位中的一个模型被摧毁，且该单位不再受战斗震撼。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-aquiline-prow",
        "name": "鹰喙舰首",
        "text": "此单位每次结束一次冲锋移动时，你可以选择一个位于其接战范围内的敌方单位，然后掷一个 D6：结果为 2-3 时，该敌方单位受到 D3 点致命伤；结果为 4-5 时，该敌方单位受到 3 点致命伤；结果为 6 时，该敌方单位受到 D3+3 点致命伤。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此军官每回合可下达 1 道命令给 REGIMENT 或 SQUADRON 单位。此军官只能下达「前进！前进！前进！」、「责任与荣誉！」和「装上刺刀！」命令。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "步行的格雷夫斯政委": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为 **星界军(Astra Militarum)，军官(Officer)** 可以下令。每份 **军官(Officer)** 数据表会说明它在一个战斗回合中能下多少令，以及哪些单位可以接收该令。每次 **军官(Officer)** 模型下令时，选择下列其中一个令，然后选择该 **军官(Officer)** 模型 6\" 内符合资格的一个友方单位接收该令。\n\n**军官(Officer)** 模型可在你的指挥阶段和它们从 **运输工具(Transport)** 中下车或被部署在战场上的阶段结束时下令。\n\n直到你的下一个指挥阶段开始为止，你选择的单位会受到该令的影响。除非另有说明，一个单位同时只能受一个令的影响（随后下达给该单位的任何令会替代现有的令）。惊吓战斗状态的单位不能接收令，如果受令单位陷入惊吓状态，该令对该单位的影响会停止。\n\n只有 **星界军(Astra Militarum)** 模型能获得下达给其单位的令的效益。\n\n冲啊！冲啊！冲啊！\n\n此单位模型的移动特征加上 3\"。\n\n刺刀冲锋！\n\n此单位模型装备的近战武器的武器技巧特征提升 1。\n\n瞄准射击！\n\n此单位模型装备的远程武器的弹道技巧特征提升 1。\n\n前列射击！后列射击！\n\n此单位模型装备的快射武器的攻击次数特征提升 1。\n\n掩蔽！\n\n此单位模型的豁免(Sv)特性提升 1（此不能将模型的豁免提升至优于 3+）。\n\n职责与荣誉！\n\n此单位模型的领导力和目标控制特征提升 1。\n\n帝国卫队军官经历培训，旨在灌输不动摇的服从。他们掌握指挥权力与尊重，以傲慢的咆哮下达令命，其声音刺穿战争的喧嚣。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-icon-of-discipline",
        "name": "纪律之印",
        "text": "此模型的单位有资格在后撤的同一战斗回合进行射击和宣告冲锋。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-brutal-disciplinarian",
        "name": "残酷纠察官",
        "text": "每回合一次，在任何阶段开始时，你可选择一个友方 **星界军(Astra Militarum)步兵(Infantry)**（不包含只含有一个模型的单位）单位，其处于战斗震撼状态且在此模型 12\"内。若你如此做，摧毁该单位中的一个模型，且该单位不再处于战斗震撼状态。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此军官每回合可下达 1 道命令给 REGIMENT 单位。此军官只能下达「前进！前进！前进！」、「责任与荣誉！」和「装上刺刀！」命令。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-using-commissar-graves",
        "name": "使用政委格雷夫斯",
        "text": "你的军队只能包含一个 COMMISSAR GRAVES 或 COMMISSAR GRAVES ON FOOT 单位。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "雅瑞克政委": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为**星界军(Astra Militarum)**，具有此能力的**军官(Officer)**可下达命令。每份**军官(Officer)**资料卡会指定其在一个战斗回合中可下达多少命令，以及哪些单位可接收该命令。每当一个**军官(Officer)**模型下达命令时，选择下方的命令之一，然后选择该**军官(Officer)**模型6寸内的一个合格友军单位来接收命令。\n\n**军官(Officer)**模型可在你的指挥阶段或其从**运输工具(Transport)**工具下车或在战场上布置的阶段结束时下达命令。\n\n直至你下一个指挥阶段开始为止，你选择的单位受到该命令影响。除非另有说明，一个单位一次只能受到一个命令影响（随后下达给该单位的任何命令取代现有命令）。命令不能下达给陷入战斗震撼的单位，若受命令影响的单位陷入战斗震撼，该命令对该单位的影响停止。\n\nOnly **星界军(Astra Militarum)** 模型得益于下达给其单位的命令。\n\n前进！前进！前进！\n\n向此单位中的模型的移动特性增加3寸。\n\n刺刀上膛！\n\n提升此单位中由模型配备的近战武器的武器技巧特性1点。\n\n瞄准！\n\n提升此单位中由模型配备的远程武器的巴利斯蒂克技巧特性1点。\n\n第一排，开火！第二排，开火！\n\n提升此单位中由模型配备的速射武器的攻击次数特性1点。\n\n寻求掩护！\n\n提升此单位中模型的豁免特性1点（此无法将模型的豁免提升至优于3+）。\n\n职责与荣誉！\n\n提升此单位中模型的领导力和目标控制特性各1点。\n\n经过训练以灌输绝对服从，帝国警卫的军官拥有司令权和尊重，以及能够穿过战争喧嚣发出的威严号令。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-will-of-iron",
        "name": "钢铁意志",
        "text": "此模型首次被消灭时，将其移出场外，然后在该阶段结束时，掷一次D6：掷出2+时，于其被消灭的位置尽可能靠近的地方将此模型重新放回战场，不得位于一个或多个敌方单位的交战范围内，且剩余3点伤害。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-hero-of-hades-hive",
        "name": "哈德斯蜂巢英雄",
        "text": "在你的指挥阶段，可选择「哈迪斯巢城英雄」章节中的一个能力。直到你的下一个指挥阶段开始前，此模型具有该能力。\n\n**反制战略家：** 在你对手移动阶段结束时，你可以选择一个于此模型 9\" 内被设置或结束移动的敌方单位，以及一个于此模型 6\" 内且为此模型可见、本阶段尚未进行正常移动或被「火力警戒(Fire Overwatch)」策略指定的友方 **军团(Regiment)** 单位。该 **军团(Regiment)** 单位接着可以选择：\n■ 进行一次最多 D6\" 的正常移动。\n■ 对该敌方单位射击，但仅限其为合法目标时。直到此阶段结束，该单位中每当一个模型进行一次远程攻击时，不论攻击武器的弹道技巧或任何修正值，皆须掷出未修正的 6 才能命中。\n■ 进行一次突进移动(Surge move)。为此掷一颗 D6：该单位中的模型移动至多等于掷骰结果的英寸数，但该单位必须以尽可能接近最近敌方单位（**飞行器(Aircraft)** 除外）的方式结束该移动。如此移动时，那些模型可以移入该敌方单位的交战范围。当该单位处于战斗震撼状态或位于一个或多个敌方单位的交战范围内时，不能进行突进移动，且每阶段仅能进行一次突进移动。\n\n**决断指挥：** 此模型发布的下一道命令可发布给此模型 12\" 内一个合法的友方单位；若这是你军队本指挥阶段中由单位发布的第一道命令，该命令不仅影响该合法单位，亦影响该合法单位 6\" 内每个其他合法的友方单位。\n\n**鼓舞英雄（光环）：** 当一个友方 **星界军(Astra Militarum)** 单位在此模型 6\" 内时，你可以为该单位重掷战斗震撼测试与领导力测试。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此军官每回合最多可下达 2 道命令给 REGIMENT、SQUADRON 或泰坦单位。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "克里格死亡军团": [
      {
        "id": "unique-grim-demeanour",
        "name": "冷酷态度",
        "text": "此单位中的模型每次进行攻击时，若此单位低于其起始兵力，从命中掷骰中加 1；若此单位低于半数，也从致伤掷骰中加 1。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "装备能力",
        "text": "■ 此单位每有 10 个模型，最多 2 名死亡军团步兵可各自将其雷射枪替换为以下之一*：\n? 1 支火焰枪\n? 1 支榴弹发射器\n? 1 支远程雷射枪\n? 1 支熔火枪\n? 1 把电浆枪\n\n*除非此单位包含 20 个模型，否则你无法选择相同选项超过一次。若包含 20 个模型，则无法选择相同选项超过两次。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 此单位每满 10 个模型，1 名配备雷射枪的死亡军团士兵可配备 1 个死亡军团医疗包（该模型的雷射枪不能被替换）。**",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 此单位每 10 个模型，1 个配备拉枪的死亡军团士兵可配备 1 个语音增幅器（该模型的拉枪不可替换）。\n\n** 一个模型只能选择这些选项之一。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 任意数量的死亡禁卫监视官可各将其轻型手枪和链刀替换为 1 支精锐枪和 1 件近战武器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 任意数量的死亡军团监督官可各自将其链剑替换为 1 强力武器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武器装备能力",
        "text": "■ 任意数量的死亡军团巡察官可各自将其雷射手枪替换为下列其一：\n? 1 栓动手枪\n? 1 电浆手枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-vox-caster",
        "name": "通讯机",
        "text": "你每次以计谋指定持有者所在的单位为目标时，掷一颗 D6，若 6\" 内有一个或多个友军 **OFFICER** 模型则结果 +1：若为 5+，你获得 1CP。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-death-korps-medi-pack",
        "name": "死亡军团医疗包",
        "text": "在你的指挥阶段开始时，若持有者所在的单位低于其起始兵力，你可以将至多 D3 个被消灭的死亡军团兵(Death Korps Troopers)返回到此单位（若此单位包含两个装备死亡军团医疗包(Death Korps medi-pack)的模型，则改为将至多 D3+1 个被消灭的死亡军团兵返回到此单位）。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "死亡骑兵": [
      {
        "id": "unique-screening-line",
        "name": "掩护防线",
        "text": "在你对手的移动阶段，若有敌方单位在此单位 8\" 内结束一次移动，且此单位不在一个或多个敌方单位的交战范围内，则此单位可进行一次距离最多 6\" 的正常移动。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "死亡直击导弹车": [
      {
        "id": "core-deadly-demise-d6",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-deathstrike-missile",
        "name": "死亡打击飞弹",
        "text": "在你的射击阶段中，每当此模型被选中进行射击时，若此模型在本场战斗中尚未使用过其死亡打击飞弹车，你可以在结算此模型的远程攻击之外，执行下列其中之一：\n\n■ **指定目标：** 若此模型的战场上没有死亡击目标标记，在战场上任何地点放置一个此模型独特的死亡击目标标记。\n\n■ **调整目标：** 若此模型的战场上已有死亡击目标标记，将该标记移动到战场上其他任何地点。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战斗装备能力",
        "text": "■ 此模型的重型枪管武器可被替换为 1 个重型火焰枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 此模型可配备1个猎杀飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "末日之锤": [
      {
        "id": "core-deadly-demise-d6-2",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-firing-deck-12",
        "name": "射击甲板12",
        "text": "某些**运输工具(Transport)**模型在其能力中列有「射击甲板 x」。每次该模型在射击阶段被选中进行射击时，你可以选择最多 x 个其单位本阶段尚未射击的已登载模型。然后，对每个已选中的登载模型，你可以选择该登载模型配备的一件远程武器（不包括具有**[单发]**能力的武器）。在该**运输工具(Transport)**模型解决所有攻击前，除其他武器外，它还计为配备所有你选中的武器。直到本阶段结束，该些已选中模型的单位不符合射击资格。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-close-range-titan-killer",
        "name": "近距离泰坦杀手",
        "text": "每当此模型的岩浆炮以敌方 **凶兽(Monster)** 或 **载具(Vehicle)** 单位为目标时，该目标始终视为在该武器的半射程范围内。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-transport",
        "name": "运载",
        "text": "该模型能够搭载 26 个星界军步兵模型。每个 OGRYN 模型占用 3 个模型的空间。该模型不能运输 ARTILLERY 模型。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "军备能力",
        "text": "■ 此模型的2个双联重型火焰枪可替换为2个双联重爆弹枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 本模型可配备以下之一：\n? 2 门雷射加农炮与 2 个双联重爆弹枪\n? 2 门雷射加农炮与 2 个双联重型火焰喷射器",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "野战炮羣": [
      {
        "id": "unique-rearm-reload-fire",
        "name": "重整、装填、射击",
        "text": "当此单位受命令影响时，若它本回合维持不动，此单位中的模型所配备的所有重型武器具有 **[连击 1]** 能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 任意数量的模型可各自将其 malleus 火箭发射器替换为以下其一：\n? 1 bombast 野战炮\n? 1 重型拉枪炮",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "冈特的幽灵": [
      {
        "id": "core-fights-first",
        "name": "先制攻击",
        "text": "拥有此能力的单位，若有资格战斗，且该单位中的所有模型都拥有此能力，则在先制攻击步骤中进行战斗。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-infiltrators",
        "name": "渗透",
        "text": "在部署期间，若一个单位中的每个模型都具有此能力，则可将其设置在战场上任何距离你对手的部署区与所有敌方单位水平超过 8\" 的位置。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-lone-operative",
        "name": "独行特工",
        "text": "除非作为一个**联合(attached)单位**的一部分，否则此单位对 12\" 以外的敌方模型**不可见**；且除非攻击模型位于此单位 12\" 内，否则不能被**[间接射击]**武器以其为目标。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-stealth",
        "name": "潜行",
        "text": "若一个单位中的每个模型都具有此能力，则每当有远程攻击以该单位为目标时，该单位在该攻击中具有**掩体增益**（13.08）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为 **星界军(Astra Militarum)，军官(Officer)** 可以下令。每份 **军官(Officer)** 数据表会说明它在一个战斗回合中能下多少令，以及哪些单位可以接收该令。每次 **军官(Officer)** 模型下令时，选择下列其中一个令，然后选择该 **军官(Officer)** 模型 6\" 内符合资格的一个友方单位接收该令。\n\n**军官(Officer)** 模型可在你的指挥阶段和它们从 **运输工具(Transport)** 中下车或被部署在战场上的阶段结束时下令。\n\n直到你的下一个指挥阶段开始为止，你选择的单位会受到该令的影响。除非另有说明，一个单位同时只能受一个令的影响（随后下达给该单位的任何令会替代现有的令）。惊吓战斗状态的单位不能接收令，如果受令单位陷入惊吓状态，该令对该单位的影响会停止。\n\n只有 **星界军(Astra Militarum)** 模型能获得下达给其单位的令的效益。\n\n冲啊！冲啊！冲啊！\n\n此单位模型的移动特征加上 3\"。\n\n刺刀冲锋！\n\n此单位模型装备的近战武器的武器技巧特征提升 1。\n\n瞄准射击！\n\n此单位模型装备的远程武器的弹道技巧特征提升 1。\n\n前列射击！后列射击！\n\n此单位模型装备的快射武器的攻击次数特征提升 1。\n\n掩蔽！\n\n此单位模型的豁免(Sv)特性提升 1（此不能将模型的豁免提升至优于 3+）。\n\n职责与荣誉！\n\n此单位模型的领导力和目标控制特征提升 1。\n\n帝国卫队军官经历培训，旨在灌输不动摇的服从。他们掌握指挥权力与尊重，以傲慢的咆哮下达令命，其声音刺穿战争的喧嚣。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-tanith-camo-cloaks",
        "name": "坦尼斯迷彩斗篷",
        "text": "此单位中的模型获得掩护利益。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-covert-stealth-team",
        "name": "隐蔽突击小队",
        "text": "在你对手的回合结束时，若此单位不在一个或多个敌方单位的交战范围内，可将此单位从战场上移除。在你下一个移动阶段的增援步骤中，将它设置在战场上任何地点，该地点距离所有敌方模型的水平距离超过 8\"。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此单位的军官每回合最多可下达 2 道命令给 REGIMENT 或冈特的幽灵单位。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "地狱之锤": [
      {
        "id": "core-deadly-demise-d6-2",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-close-quarters-warfare",
        "name": "近距离战斗",
        "text": "此模型在敌方单位处于交战距离内时不受远程攻击命中掷骰的惩罚。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 此模型的2门双联重型火焰喷射器可替换为2门双联重爆弹枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 此模型可装备以下其中之一：\n? 2 重型雷射炮和 2 双联重型机枪\n? 2 重型雷射炮和 2 双联重型火焰枪",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "地狱犬战车": [
      {
        "id": "core-deadly-demise-d6",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-flush-them-out",
        "name": "驱逐歼灭",
        "text": "在你的射击阶段中，此模型射击后，选择一个被上述攻击至少命中一次的敌方单位。直到你下一个射击阶段开始，该单位处于散乱状态。单位处于散乱状态时，不能获得掩护的好处。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战斗装备能力",
        "text": "■ 此模型的地狱火炮可被替换为下列其中之一：\n? 1 个化学炮\n? 1 个熔火炮",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 此模型的重型火焰枪可被替换为以下其中之一：\n? 1 重型机枪\n? 1 多熔焰枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型可配备 1 个猎杀飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "鹫马装甲战车": [
      {
        "id": "core-deadly-demise-1",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-convoy-escort-vehicle",
        "name": "车队护卫载具",
        "text": "在你的射击阶段中，此单位进行射击后，可进行正常移动，移动距离最多为 D6\"。若进行此移动，直到回合结束，此单位无资格宣告冲锋。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 任意数量的模型可各自将其重型枪管枪替换为 1 熔火枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 任意数量的模型可各自将其警戒者加农炮替换为以下其中一种：\n? 1门基隆盖特林加农炮\n? 1门熔火加农炮\n? 1门重型雷射加农炮",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "九头蛇自行防空炮": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-flak-battery",
        "name": "防空炮台",
        "text": "每次此模型对可 **飞行(Fly)** 的单位发动攻击时，可重掷命中掷骰。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战斗装备能力",
        "text": "■ 此模型的重爆弹枪可替换为1 重型火焰喷射器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战斗装备能力",
        "text": "■ 此模型可配备1 猎杀飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "卡舍津": [
      {
        "id": "core-scouts-6",
        "name": "侦察6\"",
        "text": "此能力一律以「斥候 X\"」的形式呈现。在结算战前能力步骤中，若一个单位中的每个模型都具有此能力，你可以执行下列其中**一**项：\n? 若该单位位于**战略预备队(strategic reserves)**中，你可以将该单位设置于完全位于你部署区内的任意位置。\n? 若该单位完全位于你的部署区内，它可以进行一次**斥候移动(scout move)**（24.32）。\n? 若该单位搭载于一个完全位于你部署区内的**专用运输工具(Dedicated Transport)**内，且搭载于该专用运输工具内的每个模型都具有斥候能力，则该专用运输工具可以进行一次斥候移动。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-warrior-elite",
        "name": "精锐战士",
        "text": "每战斗回合限定一次，在任何阶段开始时，你可以选择一个命令影响此单位直到你下一个指挥阶段开始，除本战斗回合**军官(Officer)**模型发布的任何其他命令外。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 最多 4 名卡斯金精锐步兵各可将其热射雷射枪替换为下列其一：*\n? 1 把火焰枪\n? 1 个榴弹发射器\n? 1 把热射连射枪\n? 1 把熔火枪\n? 1 把电浆枪\n\n* 每单位最多只能从此列表选择相同的武器两次",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 1 个卡斯金士兵的火焰枪可被替换为 1 挺火焰狙击枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 1 名卡斯金突击队员的高强度雷射枪可替换为 1 把高强度雷射手枪和 1 个熔火地雷。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 1 个配备热烈射线枪的卡斯金士兵可配备 1 部无线电通讯器（该模型的热烈射线枪不能被替换）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 卡斯金士官长的链刀可替换为 1 把力场武器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 卡斯金精锐队长的热战枪手枪可被替换为下列其中之一：\n? 1 把栓动手枪\n? 1 把电浆手枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-melta-mine",
        "name": "热熔地雷",
        "text": "每场战斗可进行一次，在任意阶段开始时，你可选择持有者 3\" 内的一个敌方单位并掷一颗 D6：掷出 2+ 时，该敌方单位受到 D3 致命伤，若其为 **载具(Vehicle)** 单位则改为受到 2D3 致命伤。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-vox-caster",
        "name": "通讯机",
        "text": "你每次以计谋指定持有者所在的单位为目标时，掷一颗 D6，若 6\" 内有一个或多个友军 **OFFICER** 模型则结果 +1：若为 5+，你获得 1CP。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "克里格战斗工兵": [
      {
        "id": "core-scouts-6",
        "name": "侦察6\"",
        "text": "此能力一律以「斥候 X\"」的形式呈现。在结算战前能力步骤中，若一个单位中的每个模型都具有此能力，你可以执行下列其中**一**项：\n? 若该单位位于**战略预备队(strategic reserves)**中，你可以将该单位设置于完全位于你部署区内的任意位置。\n? 若该单位完全位于你的部署区内，它可以进行一次**斥候移动(scout move)**（24.32）。\n? 若该单位搭载于一个完全位于你部署区内的**专用运输工具(Dedicated Transport)**内，且搭载于该专用运输工具内的每个模型都具有斥候能力，则该专用运输工具可以进行一次斥候移动。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-grenadiers",
        "name": "掷弹兵",
        "text": "每战斗回合一次，你可以花费 0CP 以「榴弹」策略以此单位为目标。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 1 个克里格战斗工程兵的自动手枪和壕沟棍可替换为 1 个火焰枪和 1 个近战武器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 1 位 Krieg 战斗工兵的自动手枪和壕沟俱乐部可替换为 1 把自动手枪、1 枚遥控地雷和 1 把近战武器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "装备能力",
        "text": "■ 任意数量的模型可各自将其自动手枪和战壕棍更换为1把战斗散弹枪和1件近战武器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 克里格工程师督导官的自动手枪可被替换为以下其中之一：\n? 1 把强力枪手枪\n? 1 把手持火焰枪\n? 1 把电浆手枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 克里格工兵监察官的壕沟棍可被替换为下列其中之一：\n? 1把链锯剑\n? 1把动力武器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-remote-mine",
        "name": "遥控地雷",
        "text": "每场战斗可进行一次，在你的射击阶段开始时，你可选择持有者 9\" 内且其可见的一个敌方单位并掷一颗 D6：掷出 3+ 时，该敌方单位受到 D3 致命伤，若其为 **载具(Vehicle)** 或 **防御工事(Fortifications)** 单位则改为受到 2D3 致命伤。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "克里格指挥组": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为 **星界军(Astra Militarum)**，持有此能力的 **军官(Officer)** 可以下达命令。每份 **军官(Officer)** 数据卡将指定该单位在一回合战斗中能下达多少命令，以及哪些单位符合接收资格。每当一个 **军官(Officer)** 模型下达命令时，选择下列其中一项命令，然后选择一个距该 **军官(Officer)** 模型 6\" 以内符合资格的友方单位来接收该命令。\n\n**军官(Officer)** 模型可在你的指挥阶段，以及在他们从 **运输工具(Transport)** 下车或被放置在战场上的阶段结束时下达命令。\n\n直到你下一个指挥阶段开始，被选中的单位将受该命令影响。除非另有说明，一个单位一次只能受一项命令影响（任何随后向该单位下达的命令将取代现有的命令）。命令不能下达给因战斗冲击而胆怯的单位，若受命令影响的单位变得胆怯，该命令将停止对其产生影响。\n\n只有 **星界军(Astra Militarum)** 模型才能获得向其单位下达的命令的好处。\n\n前进！前进！前进！\n\n将此单位中模型的移动特性增加 3\"。\n\n上刺刀！\n\n将此单位中模型配备的近战武器的武器技巧特性提升 1。\n\n瞄准！\n\n将此单位中模型配备的远程武器的弹道技巧特性提升 1。\n\n第一排，开火！第二排，开火！\n\n将此单位中模型配备的快速射击武器的攻击次数特性提升 1。\n\n寻找掩护！\n\n将此单位中模型的豁免特性提升 1（此无法将模型的豁免提升超过 3+）。\n\n责任与荣誉！\n\n将此单位中模型的领导力与目标控制特性各提升 1。\n\n帝国禁卫军的军官经过训练培养以灌输绝对服从，他们具备指挥威权与受人敬重，并以声势惊人的号令切穿战场的喧嚣。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-grim-determination",
        "name": "冷酷决心",
        "text": "当此单位含有 **官员** 时，即使此单位处于战斗震撼状态，仍可用策略目标此单位，且向此单位下达的命令若因此单位陷入战斗震撼而中止，不会再次中止。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此单位的军官每回合可下达 1 道命令给一个 REGIMENT 单位。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 1 资深卫兵的强力枪可替换为以下之一：\n? 1 火焰枪\n? 1 榴弹枪\n? 1 熔火枪\n? 1 电浆枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 委员长的强能武器可替换为 1 把强能拳套。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 政委长官可配备 1 电浆手枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 1 名退伍卫兵的电锯剑可被替换为以下之一：\n? 1 把壕沟棍\n? 1 把力量武器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 1 个未配备炼金术对抗剂的资深卫兵可将其拉枪替换为以下之一：\n? 1 把栓动手枪\n? 1 把等离子手枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-master-vox",
        "name": "主通讯机",
        "text": "持有者所在的单位中的 **军官(Officer)** 每次下达命令时，可以对至多 24\" 外的合格单位下达命令。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-regimental-standard",
        "name": "军团军旗",
        "text": "持有者所在的单位中的模型其目标控制(OC)特性 +1。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-alchemyk-counteragents",
        "name": "炼金术反制剂",
        "text": "持有者所在的单位对致命伤拥有不觉疼痛 6+ 能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-servo-scribes",
        "name": "伺服誊写器",
        "text": "每场战斗一次，发布命令时，Lord Commissar 可额外发布一个命令。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "克里格重型武器小队": [
      {
        "id": "unique-final-duty",
        "name": "最后职责",
        "text": "火力协调官模型在战场上时，每当一个重型武器枪手模型被摧毁，掷一次D6：结果为3+时，不将其移出游戏。该被摧毁的模型可在攻击方单位完成其攻击后射击，然后移出游戏。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-embarking",
        "name": "搭乘",
        "text": "搭乘于运输工具(Transport)内时,每个重武器射手(Heavy Weapons Gunner)模型占用 2 个模型的空间;就射击甲板(Firing Deck)能力而言,这些模型所配备的每件武器均视为 2 个模型的武器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战甲能力",
        "text": "■ 任意数量的重型武器射手可各自将其火神大炮替换为以下之一：\n? 1门克里格重型火焰喷射器\n? 1挺双联克里格重型枪炮",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "黎曼鲁斯战斗坦克": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-armoured-spearhead",
        "name": "装甲先锋",
        "text": "每当此模型对敌方单位进行攻击时，重掷命中掷骰的1，且若该单位在你不控制的目标标记范围内，你可改为重掷命中掷骰。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武器配备能力",
        "text": "■ 此模型的雷射加农炮可替换为以下其中一项：\n? 1 个重爆弹枪\n? 1 个重型火焰喷射器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 此模型可配备以下其中之一：\n? 2门重爆弹枪\n? 2门重型火焰喷射器\n? 2门多重融化枪\n? 2门电浆炮",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "装备能力",
        "text": "■ 此模型可以配备下列其中之一：\n? 1个重型机关枪\n? 1个风暴爆弹枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型可配备 1 枚猎杀飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "黎曼鲁斯指挥官": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为 **星界军(Astra Militarum)，军官(Officer)** 可以下令。每份 **军官(Officer)** 数据表会说明它在一个战斗回合中能下多少令，以及哪些单位可以接收该令。每次 **军官(Officer)** 模型下令时，选择下列其中一个令，然后选择该 **军官(Officer)** 模型 6\" 内符合资格的一个友方单位接收该令。\n\n**军官(Officer)** 模型可在你的指挥阶段和它们从 **运输工具(Transport)** 中下车或被部署在战场上的阶段结束时下令。\n\n直到你的下一个指挥阶段开始为止，你选择的单位会受到该令的影响。除非另有说明，一个单位同时只能受一个令的影响（随后下达给该单位的任何令会替代现有的令）。惊吓战斗状态的单位不能接收令，如果受令单位陷入惊吓状态，该令对该单位的影响会停止。\n\n只有 **星界军(Astra Militarum)** 模型能获得下达给其单位的令的效益。\n\n冲啊！冲啊！冲啊！\n\n此单位模型的移动特征加上 3\"。\n\n刺刀冲锋！\n\n此单位模型装备的近战武器的武器技巧特征提升 1。\n\n瞄准射击！\n\n此单位模型装备的远程武器的弹道技巧特征提升 1。\n\n前列射击！后列射击！\n\n此单位模型装备的快射武器的攻击次数特征提升 1。\n\n掩蔽！\n\n此单位模型的豁免(Sv)特性提升 1（此不能将模型的豁免提升至优于 3+）。\n\n职责与荣誉！\n\n此单位模型的领导力和目标控制特征提升 1。\n\n帝国卫队军官经历培训，旨在灌输不动摇的服从。他们掌握指挥权力与尊重，以傲慢的咆哮下达令命，其声音刺穿战争的喧嚣。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-vox-net",
        "name": "通讯网",
        "text": "每次此模型下达指令时，该指令可以发给距离最多 12\" 的合适单位。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-death-befitting-an-officer",
        "name": "军官之死",
        "text": "当此模型被摧毁时，掷一次D6：结果为2+时，不从游戏中移除此模型──在攻击方模型的单位完成其攻击后，此模型可如同处于你的射击阶段且具有满伤害值一样进行射击。然后此模型从游戏中移除。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此军官每回合最多可下达 2 道命令给 SQUADRON 单位。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 此模型的列曼鲁斯战斗炮可被替换为以下之一：\n? 1 门摧毁者战斗炮\n? 1 门根除者新星炮\n? 1 门处决者电浆炮\n? 1 门灭绝者自动炮\n? 1 门惩罚者格林机炮\n? 1 门克服者战斗炮",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 此模型的雷射加农炮可被替换为以下之一：\n? 1 重爆弹枪\n? 1 重型火焰喷射器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型可配备以下其中之一：\n? 2 具重型枪榴弹枪\n? 2 具重型火焰枪\n? 2 具重型熔火枪\n? 2 具等离子加农炮",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "配备能力",
        "text": "■ 此模型可配备下列其中一项：\n? 1 挺重型机枪\n? 1 把风暴爆弹枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型可配备 1 个猎杀飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "黎曼鲁斯破坏者": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-line-breaker",
        "name": "破坏者",
        "text": "使用其摧毁者战斗炮进行远程攻击时，此模型可以攻击在其交战范围内的敌方单位（假设没有其他友方单位也在该敌方单位的交战范围内）。此外，进行远程攻击时，此模型不会因在一个或多个敌方单位的交战范围内而受到命中掷骰的惩罚。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 本模型的雷射加农炮可替换为以下其中之一：\n? 1挺重爆弹枪\n? 1台重型火焰喷射器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战斗装备能力",
        "text": "■ 此模型可以配备以下之一：\n? 2 门重爆弹枪\n? 2 门重型火焰喷射器\n? 2 门多重融化枪\n? 2 门电浆炮",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 此模型可配备以下其中之一：\n? 1 重型枪管枪\n? 1 风暴枪栓步枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装装备能力",
        "text": "■ 此模型可配备1枚猎杀者飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "黎曼鲁斯根除者": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-urban-warfare",
        "name": "城市战",
        "text": "每次远程攻击以此模型为目标时，若此模型对该攻击有掩护加成，从该攻击的伤害特性上减1。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型的战斗加农炮可替换为下列其中之一：\n? 1 支重型栓动枪\n? 1 支重型火焰枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战甲能力",
        "text": "■ 该模型可以配备下列其中之一：\n? 2 个重爆弹枪\n? 2 个重型火焰喷射器\n? 2 个多重热熔枪\n? 2 个电浆炮",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 本模型可配备以下其中之一：\n? 1挺重型机枪\n? 1枝风暴爆弹枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 此模型可配备 1 个猎杀飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "黎曼鲁斯处决者": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-gung-ho-executioners",
        "name": "热血行刑者",
        "text": "每当此模型用其处刑者电浆炮对低于半数的单位发动攻击时，命中掷骰 +1。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 此模型的雷射加农炮可替换为下列其中之一：\n? 1 个重爆弹枪\n? 1 个重型火焰喷射器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 此模型可装备下列其中之一：\n? 2 门重爆弹枪\n? 2 门重型火焰喷射器\n? 2 门多重热熔枪\n? 2 门电浆炮",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 本模型可配备下列其中一项：\n? 1 重型机枪\n? 1 风暴爆弹枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 此模型可配备1枚猎杀飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "黎曼鲁斯灭绝者": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-withering-hail",
        "name": "凋零暴雨",
        "text": "在你的射击阶段中，此模型完成射击后，选择一个被该模型其一或多次驱逐者自动炮攻击击中的敌方单位。直到阶段结束，每次友方 **星界军(Astra Militarum)** 单位对该敌方单位发动攻击时，改善该攻击的护甲穿透特性+1。同一敌方单位每个阶段只能受此能力影响一次。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武器装备能力",
        "text": "■ 此模型的雷射加农炮可以替换为以下任一项：\n? 1门重爆弹枪\n? 1门重型火焰喷射器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型可配备以下其中之一：\n? 2 挺重型弹枪\n? 2 挺重型火焰枪\n? 2 挺多重熔火枪\n? 2 门血浆加农炮",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "护甲能力",
        "text": "■ 此模型可以配备以下之一：\n? 1 重型机关枪\n? 1 风暴爆弹枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 此模型可配备 1 猎杀飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "黎曼鲁斯惩罚者": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-mow-down-the-enemy",
        "name": "踏平敌军",
        "text": "每次此模型用其惩罚者格林机炮进行攻击，目标为敌方单位（不包括 **凶兽(Monster)** 和 **载具(Vehicle)**）时，该攻击具有 **[毁灭性创伤]** 能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 此模型的雷射加农炮可以替换为以下之一：\n? 1个重爆弹枪\n? 1个重型火焰喷射器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 此模型可配备以下之一：\n? 2 重爆弹枪\n? 2 重型火焰喷射器\n? 2 多重热熔枪\n? 2 电浆炮",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 本模型可配备下列之一：\n? 1 支重型机枪\n? 1 支风暴爆弹枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 此模型可配备1枚猎杀飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "黎曼鲁斯胜利者": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-tank-killer",
        "name": "战车杀手",
        "text": "每次此模型用其征服者战斗炮进行远程攻击，目标为 **凶兽(Monster)** 或 **载具(Vehicle)** 单位时，你可以重掷致伤掷骰。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 此模型的雷射加农炮可被替换为以下其中一项：\n? 1 重爆弹枪\n? 1 重型火焰喷射器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战斗装备能力",
        "text": "■ 此模型可配备下列之一：\n? 2 个重爆弹枪\n? 2 个重型火焰喷射器\n? 2 个多重热熔枪\n? 2 个电浆炮",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武器装备能力",
        "text": "■ 此模型可配备下列项目之一：\n? 1 架重型机枪\n? 1 架风暴爆弹枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 此模型可配备 1 枚猎杀飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "大元帅德雷尔": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-feel-no-pain-6",
        "name": "不觉疼痛6+",
        "text": "此能力一律以「不觉疼痛 X+」的形式呈现。每当具有此能力的模型将要失去一个伤值时，掷一次 D6：掷出 X+ 时，该伤值不会失去。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为**星界军(Astra Militarum)**，具有此能力的**军官(Officer)**可下达命令。每份**军官(Officer)**资料卡会指定其在一个战斗回合中可下达多少命令，以及哪些单位可接收该命令。每当一个**军官(Officer)**模型下达命令时，选择下方的命令之一，然后选择该**军官(Officer)**模型6寸内的一个合格友军单位来接收命令。\n\n**军官(Officer)**模型可在你的指挥阶段或其从**运输工具(Transport)**工具下车或在战场上布置的阶段结束时下达命令。\n\n直至你下一个指挥阶段开始为止，你选择的单位受到该命令影响。除非另有说明，一个单位一次只能受到一个命令影响（随后下达给该单位的任何命令取代现有命令）。命令不能下达给陷入战斗震撼的单位，若受命令影响的单位陷入战斗震撼，该命令对该单位的影响停止。\n\nOnly **星界军(Astra Militarum)** 模型得益于下达给其单位的命令。\n\n前进！前进！前进！\n\n向此单位中的模型的移动特性增加3寸。\n\n刺刀上膛！\n\n提升此单位中由模型配备的近战武器的武器技巧特性1点。\n\n瞄准！\n\n提升此单位中由模型配备的远程武器的巴利斯蒂克技巧特性1点。\n\n第一排，开火！第二排，开火！\n\n提升此单位中由模型配备的速射武器的攻击次数特性1点。\n\n寻求掩护！\n\n提升此单位中模型的豁免特性1点（此无法将模型的豁免提升至优于3+）。\n\n职责与荣誉！\n\n提升此单位中模型的领导力和目标控制特性各1点。\n\n经过训练以灌输绝对服从，帝国警卫的军官拥有司令权和尊重，以及能够穿过战争喧嚣发出的威严号令。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-tough-to-kill",
        "name": "难缠顽敌",
        "text": "此模型首次被摧毁时，在该阶段结束时掷 1D6。结果为 2+，将此模型重新放置在战场上，位置尽可能靠近其被摧毁的位置，且不位于任何敌方单位的交战范围内，剩余伤害为 D3。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-leading-the-charge",
        "name": "领导冲锋",
        "text": "此单位每次进行冲锋移动时，直到本回合结束，该单位内的模型装备的近战武器具有 **[毁灭性创伤]** 能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此军官每回合最多可下达 3 道命令给 REGIMENT 单位。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "太阳领主雷昂图斯": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为**星界军(Astra Militarum)**，具有此能力的**军官(Officer)**可下达命令。每份**军官(Officer)**资料卡会指定其在一个战斗回合中可下达多少命令，以及哪些单位可接收该命令。每当一个**军官(Officer)**模型下达命令时，选择下方的命令之一，然后选择该**军官(Officer)**模型6寸内的一个合格友军单位来接收命令。\n\n**军官(Officer)**模型可在你的指挥阶段或其从**运输工具(Transport)**工具下车或在战场上布置的阶段结束时下达命令。\n\n直至你下一个指挥阶段开始为止，你选择的单位受到该命令影响。除非另有说明，一个单位一次只能受到一个命令影响（随后下达给该单位的任何命令取代现有命令）。命令不能下达给陷入战斗震撼的单位，若受命令影响的单位陷入战斗震撼，该命令对该单位的影响停止。\n\nOnly **星界军(Astra Militarum)** 模型得益于下达给其单位的命令。\n\n前进！前进！前进！\n\n向此单位中的模型的移动特性增加3寸。\n\n刺刀上膛！\n\n提升此单位中由模型配备的近战武器的武器技巧特性1点。\n\n瞄准！\n\n提升此单位中由模型配备的远程武器的巴利斯蒂克技巧特性1点。\n\n第一排，开火！第二排，开火！\n\n提升此单位中由模型配备的速射武器的攻击次数特性1点。\n\n寻求掩护！\n\n提升此单位中模型的豁免特性1点（此无法将模型的豁免提升至优于3+）。\n\n职责与荣誉！\n\n提升此单位中模型的领导力和目标控制特性各1点。\n\n经过训练以灌输绝对服从，帝国警卫的军官拥有司令权和尊重，以及能够穿过战争喧嚣发出的威严号令。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-the-lord-solar",
        "name": "日光领主",
        "text": "在你的指挥阶段开始时，若此模型在战场上，你获得 1CP。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-the-collegiate-astrolex",
        "name": "星图议会",
        "text": "两名玩家都部署完军队后，从你的军队中选择最多三个 **星界军(Astra Militarum)** 单位并重新部署它们。执行时，你可将那些单位部署于战略预备队中，无论已有多少单位在战略预备队里。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此军官每回合最多可下达 3 道命令给:■ REGIMENT 单位\n■ SQUADRON 单位\n■ 泰坦单位",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-supreme-commander",
        "name": "最高统帅",
        "text": "若此模型在你的军队中,则它必须是你的统帅(Warlord)。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "蠍尾狮导弹车": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-furious-barrage",
        "name": "猛烈轰击",
        "text": "在你的射击阶段，此模型开火后，选择一个（排除 **凶兽(Monster)** 和 **载具(Vehicle)**）被此模型的风暴鹰火箭攻击命中的敌方单位。直到你的下一个射击阶段开始，该敌方单位受到迷蒙状态。当一个单位受迷蒙状态时，该单位中的模型的目标控制特性减少 1（最低为 1）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战术装备能力",
        "text": "■ 此模型的重爆弹枪可被替换为1把重型火焰喷射器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战甲能力",
        "text": "■ 此模型可装备1枚猎杀者飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "风暴军指挥组": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为 **星界军(Astra Militarum)，军官(Officer)** 可以下令。每份 **军官(Officer)** 数据表会说明它在一个战斗回合中能下多少令，以及哪些单位可以接收该令。每次 **军官(Officer)** 模型下令时，选择下列其中一个令，然后选择该 **军官(Officer)** 模型 6\" 内符合资格的一个友方单位接收该令。\n\n**军官(Officer)** 模型可在你的指挥阶段和它们从 **运输工具(Transport)** 中下车或被部署在战场上的阶段结束时下令。\n\n直到你的下一个指挥阶段开始为止，你选择的单位会受到该令的影响。除非另有说明，一个单位同时只能受一个令的影响（随后下达给该单位的任何令会替代现有的令）。惊吓战斗状态的单位不能接收令，如果受令单位陷入惊吓状态，该令对该单位的影响会停止。\n\n只有 **星界军(Astra Militarum)** 模型能获得下达给其单位的令的效益。\n\n冲啊！冲啊！冲啊！\n\n此单位模型的移动特征加上 3\"。\n\n刺刀冲锋！\n\n此单位模型装备的近战武器的武器技巧特征提升 1。\n\n瞄准射击！\n\n此单位模型装备的远程武器的弹道技巧特征提升 1。\n\n前列射击！后列射击！\n\n此单位模型装备的快射武器的攻击次数特征提升 1。\n\n掩蔽！\n\n此单位模型的豁免(Sv)特性提升 1（此不能将模型的豁免提升至优于 3+）。\n\n职责与荣誉！\n\n此单位模型的领导力和目标控制特征提升 1。\n\n帝国卫队军官经历培训，旨在灌输不动摇的服从。他们掌握指挥权力与尊重，以傲慢的咆哮下达令命，其声音刺穿战争的喧嚣。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-deep-strike",
        "name": "深入打击",
        "text": "每当此单位进行一次**进场移动(ingress move)**（20.04）时，若此单位中的每个模型都具有此能力，则可将其设置在战场上任何距离所有敌方单位水平超过 8\" 的位置，即使该位置位于你对手的部署区内。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-tempestor-prime",
        "name": "暴风突击长",
        "text": "当此单位包含暴风突击长时，此单位中的模型配备的远程武器具有 **[连击 1]** 能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此单位的军官每回合可下达 1 道命令给一个 REGIMENT 单位。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 任意数量的暴风突击兵可各自将其热枪步枪替换为以下其中之一：*\n? 1 喷焰枪\n? 1 榴弹发射器\n? 1 热枪齐射枪\n? 1 熔火枪\n? 1 电浆枪\n\n* 你每个单位最多只能从此列表中选择一次相同的武器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 1个配备热切雷枪的暴风军曹可配备1面团队标准（该模型的热切雷枪无法被替换）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 1 名暴风神射手的热射雷射枪可替换为 1 支热射雷射手枪和 1 台主通讯器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "军备能力",
        "text": "■ 1 个风暴禁军斥候的热射枪可替换为下列其中之一：\n? 1 个热射手枪和 1 个医疗包\n? 1 个热射枪、1 个热射手枪和 1 个医疗包（该模型的热射枪无法被替换）",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装装备能力",
        "text": "■ 暴风突击长的栓动手枪可替换为下列其一：\n? 1 把电浆手枪\n? 1 把指挥杖",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-master-vox",
        "name": "主通讯机",
        "text": "持有者所在的单位中的 **军官(Officer)** 每次下达命令时，可以对至多 24\" 外的合格单位下达命令。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-regimental-standard",
        "name": "军团军旗",
        "text": "持有者所在的单位中的模型其目标控制(OC)特性 +1。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-medi-pack",
        "name": "医疗包",
        "text": "在你的指挥阶段开始时，若持有者所在的单位低于其起始兵力，你可将至多 D3 个被摧毁的天谴禁军模型归还至此单位。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-command-rod",
        "name": "指挥权杖",
        "text": "当持有者正在领导一个单位时，该单位可同时受到至多两个不同的命令影响。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "国教牧师": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-war-hymns",
        "name": "战争赞歌",
        "text": "当此模型领导一个单位时，该单位中模型配备的近战武器具有 **[连击 1]** 能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-holy-piety",
        "name": "圣洁虔诚",
        "text": "此模型每次发动近战攻击时，除非此模型的单位受到战斗震撼，否则你可以重掷命中掷骰。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型的狂信者圣器可被替换为 1 把圣火手枪和 1 把动力武器。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "保镖诺克": [
      {
        "id": "core-feel-no-pain-5",
        "name": "不觉疼痛5+",
        "text": "此能力一律以「不觉疼痛 X+」的形式呈现。每当具有此能力的模型将要失去一个伤值时，掷一次 D6：掷出 X+ 时，该伤值不会失去。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-ogryn-bodyguard",
        "name": "食人魔保镖",
        "text": "当一个或多个 **军官(Officer)** 模型与此模型在同一单位时，这些 **军官(Officer)** 模型具有 4+ 不觉疼痛能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-thunderous-head-butt",
        "name": "雷霆头锤",
        "text": "每当此模型的单位被选中进行战斗时，你可以选择一个在此模型交战范围内的敌方单位并掷一次 D6：掷出 2-5 时，该敌方单位承受 D3 点致命伤；掷出 6 时，该敌方单位承受 D3+3 点致命伤。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-loyal-protector",
        "name": "忠诚护卫",
        "text": "在宣告战斗编成步骤开始时,此模型必须加入你军队中一个 COMMAND SQUAD 单位(每个 COMMAND SQUAD 至多加入一个 LOYAL PROTECTOR 模型)。此后至战斗结束,此模型视为该 COMMAND SQUAD 的一部分,且其起始兵力(Starting Strength)随之增加。若无法将此模型加入 COMMAND SQUAD,则它不参与战斗,并视为已被摧毁。当此模型加入某单位时,它可搭乘于该单位所能搭乘的任何运输工具(Transport)内,并占用 3 个模型的空间。此模型不能被选为你的统帅(Warlord)。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "欧格林保镖": [
      {
        "id": "core-feel-no-pain-6",
        "name": "不觉疼痛6+",
        "text": "此能力一律以「不觉疼痛 X+」的形式呈现。每当具有此能力的模型将要失去一个伤值时，掷一次 D6：掷出 X+ 时，该伤值不会失去。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-ogryn-bodyguard",
        "name": "食人魔保镖",
        "text": "当一个或多个 **军官(Officer)** 模型与此模型在同一单位时，这些 **军官(Officer)** 模型具有 4+ 不觉疼痛能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-loyal-protector",
        "name": "忠诚护卫",
        "text": "在宣告战斗编成步骤开始时,此模型必须加入你军队中一个 COMMAND SQUAD 单位(每个 COMMAND SQUAD 至多加入一个 LOYAL PROTECTOR 模型)。此后至战斗结束,此模型视为该 COMMAND SQUAD 的一部分,且其起始兵力(Starting Strength)随之增加。若无法将此模型加入 COMMAND SQUAD,则它不参与战斗,并视为已被摧毁。当此模型加入某单位时,它可搭乘于该单位所能搭乘的任何运输工具(Transport)内,并占用 3 个模型的空间。此模型不能被选为你的统帅(Warlord),也不能被赋予强化(Enhancement)。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武器装备能力",
        "text": "■ 此模型的撕裂枪可被替换为以下其中之一：\n? 1个手榴弹手套\n? 1把战牛锤",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装装备能力",
        "text": "■ 此模型的巨大匕首可被替换为以下其中之一：\n? 1 个蛮力盾牌\n? 1 个板状盾牌",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-brute-shield",
        "name": "蛮力盾",
        "text": "持有者拥有 4+ 无敌豁免(InSv)。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-slabshield",
        "name": "巨板盾",
        "text": "持有者的伤口(W)特性为 7。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "欧格林小队": [
      {
        "id": "core-feel-no-pain-6",
        "name": "不觉疼痛6+",
        "text": "此能力一律以「不觉疼痛 X+」的形式呈现。每当具有此能力的模型将要失去一个伤值时，掷一次 D6：掷出 X+ 时，该伤值不会失去。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-point-blank-barrage",
        "name": "近距齐射",
        "text": "每次此单位中的模型进行以最接近符合条件目标为目标的远程攻击时，改善该攻击的护甲穿透特性1点。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "灵能导师": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-malign-wardings",
        "name": "恶毒守卫",
        "text": "当此模型率领一个单位时，该单位中的模型对灵能攻击具有不觉疼痛4+ 能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-psychic-barrier",
        "name": "灵能屏障",
        "text": "在你对手的射击阶段开始时，可掷一次D6：掷出1时，此 **灵能者(Psyker)** 的单位受到D3点致命伤；掷出2+时，至该阶段结束止，此 **灵能者(Psyker)** 单位中的模型具有4+ 无敌豁免。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "莱特林": [
      {
        "id": "core-infiltrators",
        "name": "渗透",
        "text": "在部署期间，若一个单位中的每个模型都具有此能力，则可将其设置在战场上任何距离你对手的部署区与所有敌方单位水平超过 8\" 的位置。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-stealth",
        "name": "潜行",
        "text": "若一个单位中的每个模型都具有此能力，则每当有远程攻击以该单位为目标时，该单位在该攻击中具有**掩体增益**（13.08）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-shoot-sharp-and-scarper",
        "name": "精准射击后撤离",
        "text": "在你的射击阶段中，此单位射击后，若其不在任何敌方单位的交战范围内，则可进行一次常规移动，如同在你的移动阶段。若进行此操作，直到回合结束，此单位不符合宣告冲锋的资格。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 若此单位含 10 个模型，1 个模型的狙击枪可替换为 1 把坦克杀手枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 若此单位包含 10 个模型，一个模型可配备破坏装备。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 若此单位包含 10 个模型，可配备 1 鼠兵战兽。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-ratling-battlemutt",
        "name": "拉特林战犬",
        "text": "每场战斗可进行一次，当此单位被选择射击时，其可使用此能力。若如此做，直到该阶段结束，此单位中模型所装备的远程武器拥有 **[致命一击]** 能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-demolition-gear",
        "name": "爆破装备",
        "text": "持有者所在的单位拥有 **手榴弹(Grenades)** 关键字。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "罗格多恩战斗坦克": [
      {
        "id": "core-deadly-demise-d6",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-ablative-plating",
        "name": "消融装甲",
        "text": "每场战斗一次，当攻击配置给此模型时，你可将该攻击的伤害特性改为 0。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 此模型的双联战斗炮可被替换为1门压制者炮和1门同轴自动炮。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型的惩罚者格林炮可被替换为1座粉碎者炮。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装装备能力",
        "text": "■ 此模型可配备下列其一：\n? 2 挺熔火枪\n? 2 挺额外重型短枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备装备能力",
        "text": "■ 此模型可配备下列之一：\n? 2 门重型栓动步枪\n? 2 门多管熔火枪",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "罗格多恩指挥官": [
      {
        "id": "core-deadly-demise-d6",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为 **星界军(Astra Militarum)**，持有此能力的 **军官(Officer)** 可以下达命令。每份 **军官(Officer)** 数据卡将指定该单位在一回合战斗中能下达多少命令，以及哪些单位符合接收资格。每当一个 **军官(Officer)** 模型下达命令时，选择下列其中一项命令，然后选择一个距该 **军官(Officer)** 模型 6\" 以内符合资格的友方单位来接收该命令。\n\n**军官(Officer)** 模型可在你的指挥阶段，以及在他们从 **运输工具(Transport)** 下车或被放置在战场上的阶段结束时下达命令。\n\n直到你下一个指挥阶段开始，被选中的单位将受该命令影响。除非另有说明，一个单位一次只能受一项命令影响（任何随后向该单位下达的命令将取代现有的命令）。命令不能下达给因战斗冲击而胆怯的单位，若受命令影响的单位变得胆怯，该命令将停止对其产生影响。\n\n只有 **星界军(Astra Militarum)** 模型才能获得向其单位下达的命令的好处。\n\n前进！前进！前进！\n\n将此单位中模型的移动特性增加 3\"。\n\n上刺刀！\n\n将此单位中模型配备的近战武器的武器技巧特性提升 1。\n\n瞄准！\n\n将此单位中模型配备的远程武器的弹道技巧特性提升 1。\n\n第一排，开火！第二排，开火！\n\n将此单位中模型配备的快速射击武器的攻击次数特性提升 1。\n\n寻找掩护！\n\n将此单位中模型的豁免特性提升 1（此无法将模型的豁免提升超过 3+）。\n\n责任与荣誉！\n\n将此单位中模型的领导力与目标控制特性各提升 1。\n\n帝国禁卫军的军官经过训练培养以灌输绝对服从，他们具备指挥威权与受人敬重，并以声势惊人的号令切穿战场的喧嚣。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-vox-net",
        "name": "通讯网",
        "text": "此模型每次下达一条命令时，可将其下达给距离 12\" 内的符合条件的单位。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-called-shots",
        "name": "指定射击",
        "text": "每次此模型被选定射击时，在解决其攻击时，你可以重掷一次命中掷骰，可重掷一次致伤掷骰，且可重掷一次伤害掷骰。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此军官每回合最多可下达 2 道命令给 SQUADRON 单位。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "装备能力",
        "text": "■ 此模型的双联战斗炮可以被替换为 1 个 压制者炮 和 1 个 coaxial 自动炮。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型的惩罚者加特林炮可替换为 1 门碎粉者加农炮。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型可配备以下之一：\n? 2 熔火枪\n? 2 额外重型顽强枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装配置能力",
        "text": "■ 此模型可配备以下其中一项：\n? 2 挺重型栓动步枪\n? 2 挺多联熔火枪",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "侦察哨兵": [
      {
        "id": "core-deadly-demise-1",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-scouts-9",
        "name": "侦察9\"",
        "text": "此能力一律以「斥候 X\"」的形式呈现。在结算战前能力步骤中，若一个单位中的每个模型都具有此能力，你可以执行下列其中**一**项：\n? 若该单位位于**战略预备队(strategic reserves)**中，你可以将该单位设置于完全位于你部署区内的任意位置。\n? 若该单位完全位于你的部署区内，它可以进行一次**斥候移动(scout move)**（24.32）。\n? 若该单位搭载于一个完全位于你部署区内的**专用运输工具(Dedicated Transport)**内，且搭载于该专用运输工具内的每个模型都具有斥候能力，则该专用运输工具可以进行一次斥候移动。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-daring-recon",
        "name": "大胆侦察",
        "text": "在你的射击阶段开始时，选择一个在此单位18\"范围内且对此单位可见的敌方单位。直到该阶段结束，每当友方 **星界军(Astra Militarum)** 模型发动针对该单位的攻击时，重掷命中掷骰为1。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武器装备能力",
        "text": "■ 任意数量的模型可各自将其多管雷射枪替换为以下其中之一：\n? 1门自动炮\n? 1挺重型火焰枪\n? 1门Las炮\n? 1发飞弹发射器\n? 1门电浆炮",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战争装备能力",
        "text": "■ 任意数量的模型每个都可配备 1 把 Sentinel 链锯剑。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战斗装备能力",
        "text": "■ 任意数量的模型各可配备 1 枚猎手杀手飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "影剑": [
      {
        "id": "core-deadly-demise-d6-2",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-titan-killer",
        "name": "泰坦杀手",
        "text": "此模型每次使用其火山大炮进行远距攻击并指向 **凶兽(Monster)** 或 **载具(Vehicle)** 单位时，该攻击具有 **[毁灭性创伤]** 能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装装备能力",
        "text": "■ 此模型的 2 双联重型火焰喷射器可替换为 2 双联重爆弹枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战术装备能力",
        "text": "■ 此模型可配备以下之一：\n? 2 雷射加农炮与2 双联重爆弹枪\n? 2 雷射加农炮与2 双联重型火焰喷射器",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "斯莱·马博": [
      {
        "id": "core-infiltrators",
        "name": "渗透",
        "text": "在部署期间，若一个单位中的每个模型都具有此能力，则可将其设置在战场上任何距离你对手的部署区与所有敌方单位水平超过 8\" 的位置。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-lone-operative",
        "name": "独行特工",
        "text": "除非作为一个**联合(attached)单位**的一部分，否则此单位对 12\" 以外的敌方模型**不可见**；且除非攻击模型位于此单位 12\" 内，否则不能被**[间接射击]**武器以其为目标。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-stealth",
        "name": "潜行",
        "text": "若一个单位中的每个模型都具有此能力，则每当有远程攻击以该单位为目标时，该单位在该攻击中具有**掩体增益**（13.08）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-one-man-army",
        "name": "一人军队",
        "text": "每个回合最多一次，在对手的射击阶段中，当敌方单位对此模型3\"内的友军**军团**单位发动远程攻击时，在该敌方单位开火后，此模型可如同在你的射击阶段中射击，但它只能对该敌方单位目标发动攻击，且只有在该敌方单位是符合条件目标时才能执行。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-like-fighting-a-shadow",
        "name": "如同与影子战斗",
        "text": "在你的射击阶段中，此模型射击后，若其不在任何敌方单位的交战范围内，可进行一次普通移动。若如此，直到本回合结束，此模型不符合宣告冲锋的资格。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-loner",
        "name": "独来独往",
        "text": "此模型不能被选为你的统帅(Warlord)。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "风暴领主": [
      {
        "id": "core-deadly-demise-d6-2",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-firing-deck-12",
        "name": "射击甲板12",
        "text": "某些**运输工具(Transport)**模型在其能力中列有「射击甲板 x」。每次该模型在射击阶段被选中进行射击时，你可以选择最多 x 个其单位本阶段尚未射击的已登载模型。然后，对每个已选中的登载模型，你可以选择该登载模型配备的一件远程武器（不包括具有**[单发]**能力的武器）。在该**运输工具(Transport)**模型解决所有攻击前，除其他武器外，它还计为配备所有你选中的武器。直到本阶段结束，该些已选中模型的单位不符合射击资格。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-mount-up",
        "name": "登车！",
        "text": "在对手的移动阶段结束时，若此 **运输工具(Transport)** 内当前无模型登船，你可选择一个友方 **星界军(Astra Militarum)步兵(Infantry)** 单位（不包括 **火炮** 单位），该单位须完全位于此 **运输工具(Transport)** 6\"内。除非该单位在一个或多个敌方单位的交战范围内，否则其可登船至此 **运输工具(Transport)**。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-transport",
        "name": "运载",
        "text": "该模型能够搭载 40 个星界军步兵模型。每个 OGRYN 模型占用 3 个模型的空间。该模型不能运输 ARTILLERY 模型。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战术装备能力",
        "text": "■ 此模型的2门双联重型火焰枪可替换为2门双联重型火枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 此模型可配备下列之中一项：\n? 2 个雷射加农炮和 2 个双联重爆弹枪\n? 2 个雷射加农炮和 2 个双联重型火焰喷射器",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "风暴之剑": [
      {
        "id": "core-deadly-demise-d6-2",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-concussive-wave",
        "name": "震波",
        "text": "在你的射击阶段，在为此模型的风暴之剑攻城加农炮选择目标后，为该目标单位以及该单位 3\" 内的其他每个单位各掷一次 D6：掷出 5+ 时，被掷骰的单位被一道冲击波袭击。在此模型于本阶段完成对该目标单位的攻击后，每个被冲击波袭击的单位承受 D3 点致命伤。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 此模型的2门双联重型火焰喷射器可替换为2门双联重爆弹枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 此模型可配备以下其中之一：\n? 2 个雷射加农炮与 2 组双联重爆弹枪\n? 2 个雷射加农炮与 2 组双联重型火焰喷射器",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "野牛装甲车": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-rapid-deployment",
        "name": "快速部署",
        "text": "单位可在此 **运输工具(Transport)** 前进后从中下车。如此下车的单位视为于该阶段进行了正常移动，且在同一回合不能宣告冲锋，但可在其他方面正常行动。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-transport",
        "name": "运载",
        "text": "该模型能够搭载 12 个星界军步兵模型。每个 OGRYN 模型占用 3 个模型的空间。该模型不能运输 ARTILLERY 模型。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 本模型可配备1枝风暴爆弹枪。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "金牛装甲车": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-transport-support",
        "name": "运输支援",
        "text": "在你的射击阶段，此模型射击后，选择一个被该次攻击中一次或多次命中的敌军单位。直到该阶段结束，该回合从此 **运输工具(Transport)** 登陆的每个模型对该敌军单位发动的攻击都可以重掷命中掷骰。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-transport",
        "name": "运载",
        "text": "该模型能够搭载 12 个 MILITARUM TEMPESTUS INFANTRY 或 ASTRA MILITARUM INFANTRY CHARACTER 模型。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "装备能力",
        "text": "■ 此模型的金牛座战炮可替换为下列其中之一：\n? 1挺金牛格林炮\n? 1具金牛飞弹发射器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 此模型的双联陶斯热射排枪可替换为 1 挺双联自动炮。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 此模型可配备 1 把风暴枪栓枪。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "技术祭司机械教士": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-enginseer",
        "name": "工程师",
        "text": "此模型在距1个或多个友方 **星界军(Astra Militarum)载具(Vehicle)** 单位3\"内时，此模型获得独行特工能力。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-omnissiah-s-blessing",
        "name": "万机神祝福",
        "text": "在你的指挥阶段，选择一个友军 **星界军(Astra Militarum) 载具(Vehicle)** 模型，其位于此模型 3\" 内。该 **载具(Vehicle)** 模型恢复最多 D3 点失去的伤，且直到你下一个指挥阶段开始前，该 **载具(Vehicle)** 模型具有 4+ 无敌豁免。每个模型每回合只能被选择用此能力一次。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-vengeance-for-the-omnissiah",
        "name": "为万机神复仇",
        "text": "若友方 **星界军(Astra Militarum) 载具(Vehicle)** 模型在此模型 12\" 内被摧毁，直到战斗结束，此模型的恩基尼尔斧头的攻击次数特性为 6。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "风暴天鹰": [
      {
        "id": "core-deep-strike",
        "name": "深入打击",
        "text": "每当此单位进行一次**进场移动(ingress move)**（20.04）时，若此单位中的每个模型都具有此能力，则可将其设置在战场上任何距离所有敌方单位水平超过 8\" 的位置，即使该位置位于你对手的部署区内。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-precision-drop",
        "name": "精准降落",
        "text": "在你的移动阶段中，当此单位以深入打击能力被配置到战场上时，可进行精准投放。若如此，此单位可被配置到战场上任何位置，该位置必须距离所有敌方单位超过 6\" 的水平距离，但直到本回合结束，它不符合宣告冲锋的资格。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-servo-sentry",
        "name": "伺服哨卫",
        "text": "当此单位使用「深入打击」能力在战场上部署时，天鹰守卫官可用其哨兵武器（其哨兵火焰枪、哨兵榴弹发射器或哨兵高温齐射枪）进行射击。\n\n**设计师的备注：** *在此单位旁放置一个伺服机器哨兵标记以提醒你。*",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 冷卫兵执行官的哨兵火焰枪可以替换为以下之一：\n? 1 哨兵烈日齐射枪\n? 1 哨兵榴弹发射器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "配备能力",
        "text": "■ 风暴总督鹫卫兵的热枪碳步枪可被替换为以下任意一项：\n? 1 把链锯剑\n? 1 把力量武器\n? 1 把热枪手枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装装备能力",
        "text": "■ 鹫队队长可配备以下其中之一：\n? 1 把枪械手枪\n? 1 把炽热枪械手枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 1 暴风鹰旗士的热射高斯卡宾枪可替换为以下之一：\n? 1 熔火卡宾枪\n? 1 电浆卡宾枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "装备能力",
        "text": "■ 一名鹫眼狂风的热光雷射卡宾枪可被替换为 2 把热光雷射手枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装装备能力",
        "text": "■ 一名风暴裂隙中士的热聚枪可被替换为 1 热聚长枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 最多两个暴风禁卫队员可各自将其热射线枪替换为 1 把热射线手枪。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "风暴忠嗣军": [
      {
        "id": "core-deep-strike",
        "name": "深入打击",
        "text": "每当此单位进行一次**进场移动(ingress move)**（20.04）时，若此单位中的每个模型都具有此能力，则可将其设置在战场上任何距离所有敌方单位水平超过 8\" 的位置，即使该位置位于你对手的部署区内。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-storm-troopers",
        "name": "风暴突击兵",
        "text": "此单位中的模型每次发动攻击时，重掷致伤掷骰 1。若该攻击的目标是敌方单位且在某个目标标记的范围内，你可改为重掷该致伤掷骰。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战斗装备能力",
        "text": "■ 此单位每 5 个模型，最多 2 名暴风突击兵可各自将其热能枪替换为以下之一：*\n? 1 火焰枪\n? 1 榴弹发射器\n? 1 热能齐射枪\n? 1 熔火枪\n? 1 电浆枪\n\n* 除非此单位包含 10 个模型，否则不能从此表中多次选择同一武器；若包含 10 个模型，则不能选择同一武器超过 2 次。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备能力",
        "text": "■ 1名 Tempestus 精锐卫兵的高能雷射枪可替换为1把高能雷射手枪和1台通讯器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "装备能力",
        "text": "■ 风暴指挥官的链锯剑可以被替换为下列之一：\n? 1 把力量拳套\n? 1 把力量武器",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战装能力",
        "text": "■ 铁血卫士的热射雷射手枪可替换为下列其一：\n? 1 把栓式手枪\n? 1 把电浆手枪",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-vox-caster",
        "name": "通讯机",
        "text": "你每次以计谋指定持有者所在的单位为目标时，掷一颗 D6，若 6\" 内有一个或多个友军 **OFFICER** 模型则结果 +1：若为 5+，你获得 1CP。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "厄苏拉·克里德": [
      {
        "id": "core-leader",
        "name": "领袖",
        "text": "战斗开始前，在**集结军队(Muster Armies)**步骤中，对于你军队中的每个**领袖(leader)**单位与**辅助(support)**单位，你可以选择一个该单位能率领的友方**护卫单位(bodyguard)**。该单位接着会在这场战斗中率领该护卫单位，并与其组成一个**联合(attached)单位**。\n\n除非另有说明，每个护卫单位只能附着一个领袖单位与一个辅助单位。\n\n详见「联合单位」（Attached Units, 19）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-voice-of-command",
        "name": "号令",
        "text": "若你的军队阵营为**星界军(Astra Militarum)**，具有此能力的**军官(Officer)**可下达命令。每份**军官(Officer)**资料卡会指定其在一个战斗回合中可下达多少命令，以及哪些单位可接收该命令。每当一个**军官(Officer)**模型下达命令时，选择下方的命令之一，然后选择该**军官(Officer)**模型6寸内的一个合格友军单位来接收命令。\n\n**军官(Officer)**模型可在你的指挥阶段或其从**运输工具(Transport)**工具下车或在战场上布置的阶段结束时下达命令。\n\n直至你下一个指挥阶段开始为止，你选择的单位受到该命令影响。除非另有说明，一个单位一次只能受到一个命令影响（随后下达给该单位的任何命令取代现有命令）。命令不能下达给陷入战斗震撼的单位，若受命令影响的单位陷入战斗震撼，该命令对该单位的影响停止。\n\nOnly **星界军(Astra Militarum)** 模型得益于下达给其单位的命令。\n\n前进！前进！前进！\n\n向此单位中的模型的移动特性增加3寸。\n\n刺刀上膛！\n\n提升此单位中由模型配备的近战武器的武器技巧特性1点。\n\n瞄准！\n\n提升此单位中由模型配备的远程武器的巴利斯蒂克技巧特性1点。\n\n第一排，开火！第二排，开火！\n\n提升此单位中由模型配备的速射武器的攻击次数特性1点。\n\n寻求掩护！\n\n提升此单位中模型的豁免特性1点（此无法将模型的豁免提升至优于3+）。\n\n职责与荣誉！\n\n提升此单位中模型的领导力和目标控制特性各1点。\n\n经过训练以灌输绝对服从，帝国警卫的军官拥有司令权和尊重，以及能够穿过战争喧嚣发出的威严号令。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-lord-castellan",
        "name": "领主城主",
        "text": "当此模型领导一个单位时，该单位最多可以受到两个不同命令的影响。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-tactical-genius",
        "name": "战术天才",
        "text": "每战斗回合一次，你的军队中具有此能力的1个单位在友军**军团**单位12\"内时可使用此能力，该单位被该策略选中时。若执行，减少该策略使用成本的CP花费1CP。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-orders",
        "name": "命令",
        "text": "此军官每回合最多可下达 3 道命令给 REGIMENT 单位。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "女武神炮艇": [
      {
        "id": "core-deadly-demise-d6",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "core-hover",
        "name": "悬停",
        "text": "每当此单位**凌空而起(takes to the skies)**（21.03）时，不需从其**最大距离**中减去 2\"。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-airborne-insertion",
        "name": "空降投入",
        "text": "在你对手移动阶段结束时，搭乘在此 **运输工具(Transport)** 中的一个或多个单位可从其中下车。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-transport",
        "name": "运载",
        "text": "该模型能够搭载 12 个星界军步兵模型。每个 OGRYN 模型占用 3 个模型的空间。该模型不能运输 ARTILLERY 模型。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战备装备能力",
        "text": "■ 此模型的多用途雷射枪可替换为 1 门拉斯枪。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 此模型的地狱袭击飞弹可被替换为 2 组多轨火箭荚。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "武装能力",
        "text": "■ 此模型可装备 2 个重型机枪。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "双足飞龙自行迫击炮": [
      {
        "id": "core-deadly-demise-d3",
        "name": "致命破灭",
        "text": "此能力一律以「致命破灭 X」的形式呈现。每当此单位中的一个模型被**摧毁**时，在搭载于其中的单位（若有）完成其**紧急脱离移动(emergency disembark move)**之后，掷 1 颗 D6。掷出 6 时，该模型发生一次**致命破灭**：该模型 6\" 范围内的每个单位各承受「X」所表示数量的**致命伤**（若此为随机数值，则对 6\" 内的每个单位分别掷骰）。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-suppression-bombardment",
        "name": "压制炮击",
        "text": "在你的射击阶段，此模型进行射击后，选择一个敌方单位（**凶兽(Monster)** 和 **载具(Vehicle)** 除外），该单位被此模型的怀佛恩四连风暴碎片迫击炮至少一次的攻击命中。直到你的下一个射击阶段开始，该敌方单位被压制。当一个单位被压制时，该单位中的模型每次进行攻击时，命中掷骰减 1。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "护甲能力",
        "text": "■ 此模型的重爆弹枪可以替换为 1 门重型火焰喷射器。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "wargear-wargear-ability",
        "name": "战具能力",
        "text": "■ 此模型可配备 1 枚猎杀飞弹。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "德雷登的骑兵队阿提拉蛮骑兵": [
      {
        "id": "faction-指挥之声",
        "name": "指挥之声",
        "text": "-",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-骑术大师",
        "name": "骑术大师",
        "text": "当该单位进行一次**后撤移动**时，那次移动不会阻止该单位**进行射击／宣布冲锋**。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "德雷登的骑兵队指挥组": [
      {
        "id": "unique-医疗包",
        "name": "医疗包",
        "text": "您的单位拥有**不觉疼痛 6+**。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-指挥之声",
        "name": "指挥之声",
        "text": "-",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-命令",
        "name": "命令",
        "text": "该单位中的**军官**可以对**德雷登的骑兵队**单位发布最多 2 条命令。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-在那呢",
        "name": "在那呢",
        "text": "在己方射击阶段开始时，您可以选择一个位于该单位 24\" 内的敌方单位。那个敌方单位的**侦测范围** +3\"。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-兵团旗帜",
        "name": "兵团旗帜",
        "text": "该单位拥有 +1 **OC**。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-大功率通讯器",
        "name": "大功率通讯器",
        "text": "在该单位的**军官** 模型发布命令时，它可以选择位于其 24\" 内的任何有效单位。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "德雷登的骑兵队卡舍津": [
      {
        "id": "unique-精英战士-每个单位-每个战斗轮次限一次",
        "name": "精英战士（每个单位，每个战斗轮次限一次）",
        "text": "在任意阶段开始时，您可以使用本技能。若使用，选择一个命令。直到下一个己方指挥阶段开始前，除了该单位收到的任何其他命令以外，被选择的命令对该单位生效。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-指挥之声",
        "name": "指挥之声",
        "text": "-",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-热熔地雷-每个单位-每场战斗限一次",
        "name": "热熔地雷（每个单位，每场战斗限一次）",
        "text": "在任意阶段开始时，您可以使用本技能。若使用，选择一个位于该模型 3\" 内的敌方单位并掷一枚 D6：\n■ 若结果为 2+：\n■ 那个敌方单位受到 D3 处**致命伤**。\n■ 或者：如果那个敌方单位拥有**载具**关键词，则受到 2D3 处**致命伤**。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "unique-通讯器",
        "name": "通讯器",
        "text": "在您对该单位使用一个**计谋**时，掷一枚 D6，如果一个己方**军官**模型位于该单位的 6\" 内，那麽掷骰结果 +1：\n■ 若结果为 5+，您获得 1CP。",
        "status": "仅供查阅",
        "effects": []
      }
    ],
    "德雷登的骑兵队阿提拉蛮骑兵（德雷登的骑兵队阿提拉蛮骑兵）": [
      {
        "id": "unique-骑术大师",
        "name": "骑术大师",
        "text": "当该单位进行一次**后撤移动**时，那次移动不会阻止该单位**进行射击／宣布冲锋**。",
        "status": "仅供查阅",
        "effects": []
      },
      {
        "id": "faction-指挥之声",
        "name": "指挥之声",
        "text": "-",
        "status": "仅供查阅",
        "effects": []
      }
    ]
  }
};
})(typeof globalThis === 'undefined' ? this : globalThis);
