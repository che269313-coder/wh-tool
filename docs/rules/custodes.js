...
(function (root) {
  ...
  const factionRules = [
    common("martial-katah", "禁军武艺", { type: "martial-katah" }),
  ];
  ...
  const unitRules = Object.fromEntries([
    unit("图拉真元帅", [
      ...
      { id: "custodes-trajan-time-lock", name: "时间枷锁", text: "时间枷锁：每场游戏一次，近战阶段开始时，你可以选择以下效果之一生效，持续本阶段：本模型的守望者战斧 A 变为 12；本模型获得 2+特殊保护", status: supported, controls: [{ id: "mode", type: "select", label: "本次选择", options: [["none", "不启用"], ["axe", "守望者战斧 A 变为 12"], ["invulnerable", "获得 2+特殊保护"]] }], effect: { type: "time-lock" } },
    ]),
    unit("瓦雷利安连长", [
    ...
    unit("盾卫连长", [
      ...
      { id: "custodes-martial-master", name: "武艺大师", text: "武艺大师：每场游戏一次性，当本模型所在单位被选择进行近战攻击时，可以在本轮近战中同时生效两种武艺", status: supported, controls: [{ id: "secondKatah", type: "select", label: "第二种禁军武艺", options: [["none", "不启用"], ["sustained", "连击 1"], ["lethal", "致命一击"]] }], effect: { type: "second-martial-katah", requiresJoined: true, phase: "melee" } },
    ...
    unit("禁军守望者", [
      ...
      { id: "custodes-wardens-will", name: "坚定意志", text: "坚定意志：当人物领导本单位时，对本单位的攻击如果 S 大于本单位的 T 值，则造伤结果减 1", status: supported, controls: [{ id: "forceLeader", type: "checkbox", label: "强行按角色已领导本单位" }], effect: { type: "high-strength-wound-minus", requiresJoined: true } },
    ...
    unit("猎巫者", [
      ...
      { id: "custodes-witchseekers-deep", name: "深渊之女", text: "深渊之女：本模型在对抗【灵能】攻击和致命伤害时算作拥有【不知疼痛 3+】", status: supported, controls: [{ id: "psychic", type: "checkbox", label: "来袭攻击具有【灵能】关键词" }], effect: { type: "deep-daughter" } },
    ]),
    unit("灭魔教团犀牛装甲车", [
      common("rhino-core", "【核心技能】：致命破灭 D3，开火口 2"),
      common("rhino-repair", "自行修理：在每个你的指挥阶段结束时，本模型恢复一点失去的 W 值"),
      { id: "custodes-rhino-deep", name: "深渊之女", text: "深渊之女：本模型在对抗【灵能】攻击和致命伤害时算作拥有【不知疼痛 3+】", status: supported, controls: [{ id: "psychic", type: "checkbox", label: "来袭攻击具有【灵能】关键词" }], effect: { type: "deep-daughter" } },
    ]),
  ]);
